...
#define CES(x) case x: return #x
    ...
    char const *cublasGetErrorString(cublasStatus_t const &ret) {
        switch (ret) {
            ...
            default:
                return "UNKNOWN_TO_BODA_CUBLAS_ERROR_CODE";
        }
    }
...
#undef CES
    ...
    void cublas_err_chk(cublasStatus_t const &ret, char const *const func_name) {
        if (ret != CUBLAS_STATUS_SUCCESS) {
            rt_err(strprintf("%s() failed with ret=%s (%s)", func_name, str(ret).c_str(), cublasGetErrorString(ret)));
        }
    }

    void cudnn_err_chk(cudnnStatus_t const &ret, char const *const func_name) {
        if (ret != CUDNN_STATUS_SUCCESS) {
            rt_err(strprintf("%s() failed with ret=%s (%s)", func_name, str(ret).c_str(), cudnnGetErrorString(ret)));
        }
    }

    template<typename T>
    struct cudnn_create_T {
        typedef cudnnStatus_t (*p_func)(T *);
    };
    template<typename T>
    struct cudnn_destroy_T {
        typedef cudnnStatus_t (*p_func)(T);
    };
    ...
    void set_cudnn_tensor_from_dims_t(cudnn_tensor_t &cu_v, dims_t const &dims) {
        ...
        for (uint32_t i = 0; i != dims.size(); ++i) {
            dims_.push_back(dims.dims(i));
            strides.push_back(dims.strides(i));
        }
    ...
    struct culibs_wrap_t {
        ...
        void conv(rtc_func_info_t const &fi, map_str_rtc_arg_t const &arg_map) {
            ...
            for (uint32_t i = 0; i != out->dims.size(); ++i) {
                ...
                uint32_t dim_sz = 1;
                dim_t const *const mbd = biases->dims.get_dim_by_name(
                        (dn == "chan") ? "out_chan" : dn); // FIXME: maybe dim of biases should be just 'chan'?
                if (mbd) {
                    ++biases_dims_used;
                    dim_sz = mbd->sz;
                }
            ...
            cudnn_err_chk(
                    cudnnGetConvolutionForwardWorkspaceSize(cdh, cu_in.v, cu_filts.v, cu_conv.v, cu_out.v, cu_conv_algo,
            ...
            cudnn_err_chk(cudnnConvolutionForward(cdh,
                                                  ...
                                                  cu_conv.v,
                                                  cu_conv_algo,
                                                  need_scratch ? rtc->get_var_raw_native_pointer(cu_work_vn)->rp_elems()
                                                               : 0,
                                                  need_scratch,
                                                  &beta,
                                                  cu_out.v,
