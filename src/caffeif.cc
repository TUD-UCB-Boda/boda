...
namespace boda {
    ...
    void subtract_mean_and_copy_img_to_batch(p_nda_t const &in_batch, uint32_t img_ix, p_img_t const &img) {
        ...
        p_nda_float_t in_batch_float = make_shared<nda_float_t>(
                in_batch); // FIXME: checks type, but only works for float
...
#pragma omp parallel for	  
        for (uint32_t y = 0; y < ibd.dims(2); ++y) {
            for (uint32_t x = 0; x < ibd.dims(3); ++x) {
                ...
                for (uint32_t c = 0; c < 3; ++c) {
                    ...
                    in_batch_float->at4(img_ix, 2 - c, y, x) =
                            get_chan(c, pel) - float(uint8_t(u32_rgba_inmc >> (c * 8)));
                }
            }
        }
    }
    ...
    std::ostream &operator<<(std::ostream &os, synset_elem_t const &v) {
        return os << "id=" << v.id << " tag=" << v.tag;
    }
    ...
    void read_synset(p_vect_synset_elem_t out, filename_t const &fn) {
        ...
        while (!ifs_getline(fn.in, ifs, line)) {
            ...
            if (spos == string::npos) {
                rt_err("failing to parse synset line: no space found: line was '" + line + "'");
            }
            ...
            if (!tag_len) {
                rt_err("failing to parse synset line: no tag found, (comma after first space) or (first space at end of line (note: implies no command after first space)): line was '" +
                       line + "'");
            }
            synset_elem_t t;
    ...
    void run_cnet_t::setup_cnet(nesi_init_arg_t *const nia) {
        ...
        if (enable_upsamp_net) {
            ...
            if (!lp->has_convolution_param()) {
                rt_err("no non-data layers or first non-data layer of net not conv layer; don't know how to create upsampled network");
            }
            ...
            if (conv_op->has("stride")) { // scale stride if present
                ...
                for (uint32_t i = 0; i != stride.size(); ++i) {
                    if (stride[i].sz & 1) {
                        rt_err("first conv layer has odd stride in some dim;don't know how to create upsampled network");
                    }
                    stride[i].sz /= 2;
                }
                stride.calc_strides();
            ...
            create_upsamp_layer_weights(conv_pipe, net_param->layer(upsamp_layer_ix).name(),
                                        conv_pipe_upsamp, upsamp_net_param->layer(
                            upsamp_layer_ix).name()); // sets weights in conv_pipe_upsamp->layer_blobs
            ...
            for (uint32_t bc = 0; bc < out_chans; ++bc) {
                for (int32_t by = i->feat_box.p[0].d[1]; by < i->feat_box.p[1].d[1]; ++by) {
                    for (int32_t bx = i->feat_box.p[0].d[0]; bx < i->feat_box.p[1].d[0]; ++bx) {
                        ...
                        valid_in_xy =
                                valid_in_xy * u32_to_i32(nominal_in_sz) / u32_to_i32(i->img_sz); // scale for scale
    ...
    void cnet_predict_t::setup_scale_infos(uint32_t const &interval, vect_u32_pt_t const &sizes,
        ...
        for (uint32_t six = 0; six < sizes.size(); ++six) {
            ...
            scale_infos.push_back(
                    scale_info_t{sz, 0, bix, dest, valid_feat_box, valid_feat_img_box}); // note: from_upsamp_net=0
            ...
            if (enable_upsamp_net && (six < interval)) {
                ...
                scale_infos[six] = scale_info_t{sz, 1, bix, dest, valid_feat_box,
                                                valid_feat_img_box}; // note: from_upsamp_net=1
            }
        }
        ...
        for (int32_t by = shrunk_quant_img_box.p[0].d[1]; by < shrunk_quant_img_box.p[1].d[1]; ++by) {
            for (int32_t bx = shrunk_quant_img_box.p[0].d[0]; bx < shrunk_quant_img_box.p[1].d[0]; ++bx) {
                ...
                if (do_set) { nms_grid[pel] = img_box; }
                    ...
                    if (i != nms_grid.end()) {
                        ++over_pel;
                        if (i->second == center_match) { ++center_match_cnt; }
                    }
                }
            }
        }
    ...
    p_vect_anno_t cnet_predict_t::pred_state_to_annos(bool const print_to_terminal) {
        ...
        for (vect_uint32_t::const_iterator ii = disp_list.begin(); ii != disp_list.end(); ++ii) {
            ...
            if (nms_box ==
                i32_box_t{}) { continue; } //nms suppression condition: overlaps other core and no close-center-match
            ...
            if (anno_mode == 0) {
                ...
                anno_str = strprintf("%-4s %s\n", str(ii - disp_list.begin() + 1).c_str(),
                                     str(out_labels->at(ps.label_ix).tag).c_str());
            } else if (anno_mode == 2) {
        ...
        for (anno_map_t::const_iterator i = annos.begin(); i != annos.end(); ++i) {
            ...
            anno.box = i->first;
            anno.fill = 0;
            anno.box_color = rgba_to_pel(170, 40, 40);
            anno.str_color = rgba_to_pel(220, 220, 255);
        }
        return ret_annos;
    }
    ...
    void cnet_predict_t::do_predict_region(p_nda_float_t const &out_batch, dims_t const &obb, dims_t const &obe,
...

        {
            uint32_t pel_ix = 0;
            uint32_t psix = psb;
            for (dims_iter_t di(obb, obe);; ++psix, ++pel_ix) {
        ...
        for (uint32_t i = 0; i != num_pels; ++i) {
            if ((fabs(pel_sums[i] - 1.0) < .01) && (pel_maxs[i] < 1.01)) {
                pel_is_pdf[i] = 1;
                ++tot_num_pdf;
            }
        ...
        {
            uint32_t pel_ix = 0;
            for (uint32_t psix = psb; psix != psb + num_pred; ++psix, ++pel_ix) {
                if (pel_ix == num_pels) { pel_ix = 0; }
        ...
        // temportal filtering and setting to_disp
        {
            ...
            for (uint32_t psix = psb; psix != psb + num_pred; ++psix, ++pel_ix) {
                ...
                if (!ps.filt_prob_init) {
                    ps.filt_prob_init = 1;
                    ps.filt_prob = ps.cur_prob;
                }
                else {
                    ps.filt_prob *= (1 - filt_rate);
                    ps.filt_prob += ps.cur_prob * filt_rate;
                }
