...
namespace boda {
    ...
    inline void bwrite(STREAM &out, rtc_arg_t const &o) {
        bwrite(out, o.n);
        bwrite(out, o.v);
    }

    template<typename STREAM>
    inline void bread(STREAM &in, rtc_arg_t &o) {
        bread(in, o.n);
        bread(in, o.v);
    }

    template<typename STREAM>
    inline void bwrite(STREAM &out, rtc_compile_opts_t const &o) {
    ...
    template<typename STREAM>
    inline void bread(STREAM &in, rtc_compile_opts_t &o) {
    ...
    template<typename STREAM>
    inline void bwrite(STREAM &out, rtc_func_call_t const &o) {
    ...
    template<typename STREAM>
    inline void bread(STREAM &in, rtc_func_call_t &o) {
    ...
    template<typename STREAM>
    inline void bwrite(STREAM &out, op_base_t const &o) {
        bwrite(out, o.str_vals);
        bwrite(out, o.nda_vals);
    }

    template<typename STREAM>
    inline void bread(STREAM &in, op_base_t &o) {
        bread(in, o.str_vals);
        bread(in, o.nda_vals);
    }

    template<typename STREAM>
    inline void bwrite(STREAM &out, rtc_func_info_t const &o) {
    ...
    template<typename STREAM>
    inline void bread(STREAM &in, rtc_func_info_t &o) {
        ...
        // bases=["rtc_compute_t"], type_id="ipc" )
    {
        ...
        void create_var_with_dims(string const &vn, dims_t const &dims) {
            ...
            bwrite(*worker, string("create_var_with_dims"));
            bwrite(*worker, vn);
            bwrite(*worker, dims);
            worker->flush();
        }
        ...
        void create_var_with_dims_as_reshaped_view_of_var(string const &vn, dims_t const &dims, string const &src_vn) {
            ...
            bwrite(*worker, vn);
            bwrite(*worker, dims);
            bwrite(*worker, src_vn);
            worker->flush();
        }

        void release_var(string const &vn) {
            must_erase(*vis, vn);
            bwrite(*worker, string("release_var"));
            bwrite(*worker, vn);
            worker->flush();
        }
        ...
        void set_var_to_zero(string const &vn) {
            bwrite(*worker, string("set_var_to_zero"));
            bwrite(*worker, vn);
            worker->flush();
        }
        ...
        virtual float get_var_ready_delta(string const &vn1, string const &vn2) {
            assert_st(0);
        } // not-yet-used-iface at higher level
        void release_func(string const &func_name) {
            bwrite(*worker, string("release_func"));
            bwrite(*worker, func_name);
            worker->flush();
        }

        uint32_t run(rtc_func_call_t const &rfc) {
            bwrite(*worker, string("run"));
            bwrite(*worker, rfc);
            worker->flush();
            uint32_t ret = 0;
            string err_str;
        ...
        void finish_and_sync(void) {
            bwrite(*worker, string("finish_and_sync"));
            worker->flush();
        }

        void release_per_call_id_data(void) {
            bwrite(*worker, string("release_per_call_id_data"));
            worker->flush();
        }

        void release_all_funcs(void) {
            bwrite(*worker, string("release_all_funcs"));
            worker->flush();
        }

        void profile_start(void) {
            bwrite(*worker, string("profile_start"));
            worker->flush();
        }

        void profile_stop(void) {
            bwrite(*worker, string("profile_stop"));
            worker->flush();
        }
    };
        ...
        virtual void main(nesi_init_arg_t *nia) {
            ...
            while (1) {
                ...
                if (0) {}
                    ...
                    vect_rtc_func_info_t func_infos;
                    rtc_compile_opts_t opts;
                    bread(*parent, func_infos);
                    bread(*parent, opts);
                    uint32_t ret = 0;
                    string err_str;
                    try {
                    ...
                    catch (unsup_exception const &rte) {
                        ret = 1;
                        err_str = rte.what();
                    } // FIXME: stacktrace lost
                    ...
                    bwrite(*parent, ret);
                    parent->flush();
                } else if (cmd == "release_func") {
                    string func_name;
                    bread(*parent, func_name);
                    rtc->release_func(func_name);
                }
                else if (cmd == "run") {
                    rtc_func_call_t rfc;
                    bread(*parent, rfc);
