...
namespace boda {
        ...
        // bases=["data_stream_t"], type_id="start-stop-skip")
    {
        ...
        virtual string get_pos_info_str(void) {
            return strprintf("tot_num_read=%s; src info: %s", str(tot_num_read).c_str(),
                             str(src->get_pos_info_str()).c_str());
        }
        ...
        virtual void data_stream_init(nesi_init_arg_t *nia) {
            ...
            for (uint32_t i = 0; i != start_block; ++i) {
                src->proc_block(data_block_t());
            } // skip to start block // FIXME: use seek here? prob. not.
        }
    };
        ...
        // bases=["data_stream_t"], type_id="ts-merge")
    {
        ...
        virtual bool seek_to_block(uint64_t const &frame_ix) {
            ...
            if (!ts_src->seek_to_block(frame_ix)) {
                assert_st(
                        0); // FIXME: we need to roll back the seek that worked above here -- but with no 'tell' that's hard ..,
            }
            return true;
        }
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            ...
            if ((!ret.valid()) ||
                (!ts_db.valid())) { return data_block_t(); } // if either stream is ended/invalid, silently give ... not ideal?
            if (ret.frame_ix != ts_db.frame_ix) {
                rt_err(strprintf(
                        "refusing to apply timestamp since stream frame_ix's don't match: data_src frame_ix=%s ts_src frame_ix=%s\n",
                        str(ret.frame_ix).c_str(), str(ts_db.frame_ix).c_str()));
            }
            if (ret.timestamp_ns != uint64_t_const_max) {
                rt_err(strprintf(
                        "refusing to apply timestamp since data stream already has timestamp: data_src timestamp_ns=%s\n",
        ...
        // bases=["data_stream_file_t"], type_id="qt")
    {
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            ...
            if (!mfsr.can_read(sizeof(uint32_t))) {
                rt_err("qt stream: read timestamp, but not enough data left to read payload size");
            }
            mfsr.read_val(ret);
        ...
        // bases=["data_stream_file_t"], type_id="dumpvideo")
    {
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            ...
            uint32_t block_sz;
            if (!mfsr.can_read(
                    sizeof(block_sz))) { return ret; } // not enough bytes left for another block. FIXME: should be an error?
        ...
        // bases=["data_stream_file_t"], type_id="text-src")
    {
        ...
        bool set_timestamp_from_text_line(data_block_t &v) {
            ...
            if (!(timestamp_fix < parts.size()) || !(frame_ix_fix < parts.size())) {
                rt_err(strprintf(
                        "can't parse timestamp and frame_ix from fields %s and %s; line had %s fields; full line=%s\n",
                        str(timestamp_fix).c_str(), str(frame_ix_fix).c_str(), str(parts.size()).c_str(),
                        str(line).c_str()));
            }
        ...
        // bases=["data_sink_file_t"], type_id="text-sink")
    {
        ...
        virtual string get_pos_info_str(void) {
            return string("data_sink_text: wrote <NOT_IMPL> lines to:") + data_sink_file_t::get_pos_info_str();
        }
        ...
        // bases=["data_stream_file_t"], type_id="csv-src")
    {
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            ...
            while (!mfsr.at_eof()) { // gather block of same-ts lines
                ...
                if (ret.timestamp_ns ==
                    uint64_t_const_max) { ret.timestamp_ns = line_ts_ns; } // first line sets block ts
                ...
                if (max_parts && (max_parts != parts.size())) {
                    ...
                    rt_err(strprintf(
                            "row with different # of fields: on first line there were %s fields, but on this line parts.size()=%s\n",
            ...
            if (max_parts < 2) {
                rt_err("unhandled: csv file with no non-timestamp fields (i.e. 0/no data fields per timestamp)");
            }
            ret.nda = make_shared<nda_t>(
                    dims_t{vect_uint32_t{(uint32_t) ts_lines_parts.size(), max_parts - 1}, {"obj", "attr"}, "float"});
    ...
    uint64_t ts_delta(data_block_t const &a, data_block_t const &b) {
        return (a.timestamp_ns > b.timestamp_ns) ? (a.timestamp_ns - b.timestamp_ns) : (b.timestamp_ns -
                                                                                        a.timestamp_ns);
    }
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            if (db.subblocks) {
                if (db.num_subblocks() != streams.size()) {
                    rt_err(strprintf(
                            "data_stream_merge: input data block must either have no subblocks (distribute-clone case), or have db.num_subblocks()=%s be equal to streams.size()=%s (which it is not).\n",
        ...
        // bases=["data_stream_t"], type_id="flatten")
    {
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            ...
            for (uint32_t i = 0; i != db.subblocks->size(); ++i) {
                ...
                if (i == 0) {
                    ret.timestamp_ns = stream_out.timestamp_ns;
                    ret.frame_ix = stream_out.frame_ix;
                }
                else {
    ...
    {
        virtual cinfo_t const *get_cinfo(void) const; // required declaration for NESI support
        p_uint32_t fold_targ; //NESI(help="fold target (if not specified, drop)")
        uint32_t fold_src; //NESI(default="0",help="fold src")

        virtual void data_stream_init(nesi_init_arg_t *const nia) {
            if (fold_targ) {
                if ((*fold_targ) == fold_src) {
                    rt_err(strprintf("fold_targ=%s and fold_src=%s must be different\n", str(fold_targ).c_str(),
                                     str(fold_src).c_str()));
                }
            }
        }
        ...
        void check_has_subblock(data_block_t const &db, uint32_t const &sbix, string const &tag) {
            if (!(sbix < db.num_subblocks())) {
                rt_err(strprintf(
                        "data_stream_fold: can't extract subblock for '%s' with index %s from input data block with db.num_subblocks()=%s.\n",
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            ...
            if (fold_targ) { check_has_subblock(db, *fold_targ, "fold_targ"); }
        ...
        // bases=["data_stream_t"], type_id="stamp")
    {
        ...
        void check_has_subblock(data_block_t const &db, uint32_t const &sbix, string const &tag) {
            if (!(sbix < db.num_subblocks())) {
                rt_err(strprintf(
                        "data_stream_stamp: can't extract subblock for '%s' with index %s from input data block with db.num_subblocks()=%s.\n",
                        tag.c_str(), str(sbix).c_str(), str(db.num_subblocks()).c_str()));
            }
        }
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            ...
            if (also_stamp_parent) {
                data_block_t ret = db;
                ret.timestamp_ns = src_sb.timestamp_ns;
                return ret;
            }
            return db;
        }
        ...
        // bases=["data_stream_t"], type_id="sync")
    {
        ...
        virtual void data_stream_init(nesi_init_arg_t *const nia) {
            ...
            next_frame_ix = 0;
            if (!(psix < streams.size())) {
                rt_err(strprintf("psix=%s must be < streams.size()=%s\n",
        ...
        virtual bool seek_to_block(uint64_t const &frame_ix) {
            for (uint32_t i = 0; i != seek_buf.size(); ++i) {
                if (seek_buf[i].frame_ix == frame_ix) {
                    seek_buf_pos = i;
                    return true;
                }
            }
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            ...
            while (1) {
                ...
                for (uint32_t i = 0; i != streams.size(); ++i) {
                    ...
                    assert_st(
                            i_dbs[0].valid()); // tail should always be valid since we require all streams to be non-empty
                    ...
                    if (max_delta_ns && (ts_delta(pdb, sdb) > max_delta_ns)) {
                        if (verbose) {
                            printf("*** no current-enough secondary block found. skipping primary block.\n");
                        }
                        ret_valid = 0;
                    } else {
                ...
                if (ret_valid) {
                    ...
                    assert_st(seek_buf.size() ==
                              seek_buf_pos); // if we're reading a block, we should be at end of seek_buf
                    if (seek_buf_size) { // otherwise, we'd try to pop_front() on a empty buf to make room ...
                        assert_st(seek_buf.size() <= seek_buf_size);
                        if (seek_buf.size() ==
                            seek_buf_size) { seek_buf.pop_front(); } // if buf was full, free a slot ...
        ...
        // bases=["data_stream_t"], type_id="pipe")
    {
        ...
        virtual void set_opt(data_stream_opt_t const &opt) {
            for (uint32_t i = 0; i != pipe.size(); ++i) { pipe[i]->set_opt(opt); }
        }
        ...
        virtual bool seek_to_block(uint64_t const &frame_ix) {
            ...
            if (ret) {
                ...
                for (uint32_t i = 0; i != pipe.size(); ++i) {
                    if (have_more_out[i]) {
                        rt_err("unimplemented: can't seek on pipe when some stage has set have_more_info. could add flush functionality to fix ...");
                    }
                }
            }
            return ret;
        ...
        // bases=["data_stream_t"], type_id="crop")
    {
        ...
        virtual string get_pos_info_str(void) {
            return strprintf("data_sink_crop: crop=%s <no-state>", str(crop).c_str());
        }

        virtual data_block_t proc_block(data_block_t const &db) {
            data_block_t ret = db;
            dims_t const &in_dims = db.nda->dims;
            if (crop.p[0].d[0] || crop.p[1].d[0]) {
                rt_err("TODO: non-zero x-coord crop for to_nda (needs copy and/or padded/strided nda support)");
            }
            ...
            uint8_t *const cropped_rp_data =
                    (uint8_t *) db.nda->rp_elems() + crop.p[0].d[1] * in_dims.tsz() * in_dims.strides(0);
    ...
    struct nda_adj_and_normalize_angle_in_degrees_t {
        ...
        nda_adj_and_normalize_angle_in_degrees_t(double const &adj_, bool const &negate_) : adj(adj_),
                                                                                            negate(negate_) {}

        template<typename T>
        void op(nda_t &nda) const {
        ...
        // bases=["has_main_t"], type_id="scan-data-stream")
    {
        ...
        void main(nesi_init_arg_t *nia) {
            ...
            while (1) {
                ...
                if (timestamp_check &&
                    db.nda.get()) { // if db has data, check timestamp (old non-multi-stream-scan functionality)
                    if (db.timestamp_ns <= last_ts) {
                        printf("**ERROR: ts did not increase: db.timestamp_ns=%s last_ts=%s stream->get_pos_info_str()=%s\n",
                               str(db.timestamp_ns).c_str(), str(last_ts).c_str(),
                               str(stream->get_pos_info_str()).c_str());
                    }
