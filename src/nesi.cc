...
namespace boda {
    ...
    void
    nesi_init_and_check_unused_from_lexp(p_lexp_t const &lexp, nesi_init_arg_t *parent, tinfo_t const *ti, void *o) {
    ...
    struct nesi_frame_t {
        ...
        void clear(void) {
            parent_xn.reset();
            os_sz_orig = uint32_t_const_max;
            os_sz_val_begin = uint32_t_const_max;
        }
    ...
    void init_var_from_nvm(nesi_init_arg_t *nia, vinfo_t const *const vi, void *rpv) {
        ...
        if (!di && vi->default_val) {
            di = parse_lexp(vi->default_val);
            found_scope = nia;
        }
    ...
    void nesi_struct_vars_help_rec(void *o, string *const os, string &prefix, cinfo_t const *ci, bool const show_all,
        ...
        for (uint32_t i = 0; ci->vars[i].exists(); ++i) {
            ...
            if (vi->req) {
                req_opt_def = "(required)";
                assert_st(!vi->default_val);
            }
    ...
    void nesi_struct_nesi_help(tinfo_t const *tinfo, void *o, string *os, string &prefix,
        ...
        if ((help_ix < help_args->size()) && (help_args->at(help_ix) != deep_help_str)) { // use a help-arg to decend
            ...
            if (!vi) {
                *os += strprintf(
                        "struct '%s' has no field '%s', so help cannot be provided for it. Ignoring remaining args and showing help here.\n",
    ...
    void nesi_struct_nesi_dump_rec(nesi_frame_t &nf, nesi_dump_buf_t *ndb, cinfo_t const *ci, void *o) {
        ...
        for (uint32_t i = 0; ci->vars[i].exists(); ++i) {
            ...
            if ((!var_at_default) && // printed field, and ( no default or not equal to default ). keep new part of os.
                ((!vi->default_val) ||
                 strncmp(vi->default_val, &(ndb->os)[nf.os_sz_val_begin], ndb->os.size() - nf.os_sz_val_begin))) {
                ndb->commit_list_val(nf);
    ...
    void leaf_type_nesi_help(tinfo_t const *tinfo, void *o, string *os, string &prefix,
        ...
        if (help_ix < help_args->size()) { // use a help-arg to decend
            if (help_args->at(help_ix) != deep_help_str) { // the 'deep help' case on leaf types doesn't make sense
                *os += strprintf(
                        "%sleaf type '%s' has no fields at all, certainly not '%s', so help cannot be provided for it.\n",
    ...
    // i32_pt_t
    init_t *nesi_i32_pt_t_init = &nesi_str_parts_init<i32_pt_t>;
    make_p_t *i32_pt_t_make_p = &has_def_ctor_make_p<i32_pt_t>;
    vect_push_back_t *i32_pt_t_vect_push_back = &has_def_ctor_vect_push_back_t<i32_pt_t>;
    nesi_dump_t *i32_pt_t_nesi_dump = &with_op_left_shift_nesi_dump<i32_pt_t>;
    void *i32_pt_t_init_arg = (void *) "i32_pt_t (pair of 32-bit signed integers, i.e. a signed point)";
    ...
    void nesi_init_checked(void *ret, tinfo_t const *const &tinfo,
        ...
        try { tinfo->init(&nvm, tinfo, ret); }
            ...
            rte.err_msg = err_prefix + rte.err_msg;
            throw;
        }
    }
    ...
    void nesi_dims_t_init(nesi_init_arg_t *nia, tinfo_t const *tinfo, void *o) {
        ...
        if (l->leaf_val.exists()) {
            ...
            rt_err("invalid attempt to use string as name/value list for " + string(tstr) + " init. string was:" +
                   str(*l));
        }
        ++l->use_cnt;
        ...
        void op(nda_t &nda) const {
            ...
            for (uint32_t i = 0; i != nda.elems_sz(); ++i) {
                try { elems[i] = boost::lexical_cast<T>(parts[i]); }
                    ...
                    rt_err(strprintf("can't convert '%s' to %s.",
    ...
    void nesi_nda_t_init(nesi_init_arg_t *nia, tinfo_t const *tinfo, void *o) {
        ...
        ++l->use_cnt;
        p_lexp_t tn, dims, vv;
        for (vect_lexp_nv_t::iterator i = l->kids.begin();
             i != l->kids.end(); ++i) { // gather fields, error check for extras
            if (0) {}
            else if (i->n.str() == "tn") { tn = i->v; }
            else if (i->n.str() == "dims") { dims = i->v; }
            else if (i->n.str() == "v") { vv = i->v; }
            else {
                rt_err("nda_t has no field named " + i->n.str() +
                       ". valid fields are 'tn' (string; type name), 'dims' (dims_t), and 'v' (string; space-or-colon-seperated list of values");
            }
        }
        if ((!dims) && (!tn)) {
            rt_err("nda_t has neither 'tn' nor 'dims' field, can't determine type; set at least one.");
        }
        string tn_str;
        if (dims) {
            ...
            try { nesi_dims_t_init(&nvm, &tinfo_dims_t, &v->dims); }
                ...
                rte.err_msg = "nda_t dims: " + rte.err_msg;
                throw;
            }
        ...
        if (vv) {
            ...
            if (!dims) {
                ...
                } // assume 1 value should be a scalar
                else {
            ...
            if (v->dims.dims_prod() != parts.size()) {
                ...
                rt_err(strprintf(
                        "nda_t 'v' field has %s elements/parts, but user-specified 'dims' say there should be %s.",
    ...
    // map_str_T
    template<typename T>
    void nesi_map_str_T_init(nesi_init_arg_t *nia, tinfo_t const *tinfo, void *o) {
        ...
        if (l->leaf_val.exists()) {
            ...
            rt_err("invalid attempt to use string as name/value list for map str -> " + string(tstr) +
                   " init. string was:" + str(*l));
        }
        ++l->use_cnt;
        for (vect_lexp_nv_t::iterator i = l->kids.begin(); i != l->kids.end(); ++i) {
            shared_ptr<T> p_vv;
