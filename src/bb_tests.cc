...
namespace boda {
    ...
    vect_string bb_test_fns = {"%(boda_test_dir)/regfile.txt", "/etc/passwd", "/etc/shadow", "/dev", "/dev/null",
                               "/dev/null/baz", "/bin/sh", "fsdlkfsjdflksjd234234"};
              ...
              public has_main_t // NESI(help="NESI wrapper for low-level boda tests; use test_boda_base() global func to run w/o NESI", bases=["has_main_t"], type_id="test_boda_base", hide=1 )
    {
        ...
        void ma_p_t1(void) {
            p_uint8_t p = ma_p_uint8_t(100, 3);
            assert_st(p);
        }

        void ma_p_t2(void) {
            p_uint8_t p = ma_p_uint8_t(100, 1024);
            assert_st(p);
        }
        ...
        void load_img_tst(void) {
            img_t img;
            img.load_fn(cur_fn);
        }

        void za_img_tst(void) {
            img_t img;
            img.set_sz_and_alloc_pels({2, 0});
        }

        void xml_gr_tst(void) {
            xml_document doc;
            xml_node xn = xml_file_get_root(doc, cur_fn);
            xml_must_decend(cur_fn, xn, "bar");
        }

        void lexp_unused_t1(void) {
            p_lexp_t l = parse_lexp("(foo=bar)");
            ++l->use_cnt;
            ++l->kids.front().v->use_cnt;
            vect_string path;
            lexp_check_unused(l.get(), path);
        }

        void lexp_unused_t2(void) {
            p_lexp_t l = parse_lexp("(foo=bar)");
            ++l->use_cnt;
            vect_string path;
            lexp_check_unused(l.get(), path);
...
#define TND(s) #s, &boda_base_test_run_t::s
        ...
        void main(nesi_init_arg_t *nia) {
            ...
            test_run(TND(eid_t3),
                     "error: error while trying to create '/dev/null' directory: boost::filesystem::create_directory: File exists: \"/dev/null\"");
            test_run(TND(eid_t4),
                     "error: error while trying to create '/dev/null/baz' directory: boost::filesystem::create_directory: Not a directory: \"/dev/null/baz\""); // note: error not the best ...
            ...
            test_run_tfns(TND(mapfnro_tst), "00111101",
                          "error: failed to open/map file '%1$s' (expanded: '%1$s') for reading");
            test_run(TND(boda_main_t1),
                     "error: specified mode name '(foo=biz)' parses as a list, and it must not be a list.");
...
FIELDS:
  ...
  out_fn: (default='%(boda_output_dir)/test_vst.txt') type=filename_t  --  output: print out nda, if specified.
            ...
            test_run_ifns(TND(load_img_tst), "--1--",
                          "error: failed to load image '%s': lodepng decoder error 27: PNG file is smaller than a PNG header");
            test_run_ifns(TND(load_img_tst), "---1-",
                          "error: failed to load image '%s': tjDecompressHeader2 failed:Not a JPEG file: starts with 0x64 0x73");
            test_run_ifns(TND(load_img_tst), "----1",
                          "error: failed to load image '%s': could not auto-detect file-type from extention. known extention/types are: '.jpg':jpeg '.png':png");
            test_run(TND(za_img_tst), "error: can't create zero-area image. requests WxH was 2x0");
            test_run_fns(bb_xml_fns, TND(xml_gr_tst), "0-1",
                         "error: loading xml file '%s' failed: Error parsing start element tag");
            test_run_fns(bb_xml_fns, TND(xml_gr_tst), "-1-",
                         "error: error: parsing xml file: '%s': expected to find child named 'bar' from node with name 'root'");
