...
namespace boda {
    ...
    i32_box_t box_from_sdl(SDL_Rect const &b) {
        return i32_box_t{{b.x, b.y},
                         {b.x + b.w, b.y + b.h}};
    }

    SDL_Color color_to_sdl(uint32_t const &c) {
        return SDL_Color{get_chan(0, c), get_chan(1, c), get_chan(2, c), get_chan(3, c)};
    }
...
#undef DECL_MAKE_P_SDL_OBJ
    ...
    struct YV12_buf_t {
        ...
        p_img_t as_img_t_yuv_borrowed(void) const {
            ...
            yuv_ndas.push_back(make_shared<nda_uint8_t>(
                    dims_t{vect_uint32_t{uint32_t(h), uint32_t(w)}, vect_string{"y", "x"}, "uint8_t"},
                    p_uint8_t(Y, null_deleter<uint8_t>())));
            yuv_ndas.push_back(make_shared<nda_uint8_t>(
                    dims_t{vect_uint32_t{uint32_t(h / 2), uint32_t(w / 2)}, vect_string{"y", "x"}, "uint8_t"},
    ...
    p_layout_elem_t
    make_p_layout_elem_t_init_and_check_unused_from_lexp(p_lexp_t const &lexp, nesi_init_arg_t *const nia);
    ...
    void disp_win_t::disp_setup(p_vect_p_img_t const &imgs_) {
        ...
        if (!disp_layout) {
            ...
            for (uint32_t i = 0; i != imgs->size(); ++i) {
                default_disp_layout_str += string(i ? "," : "") + "_=(mode=leaf:" + str(i) + ",kids=())";
            }
        ...
        if (window_sz == u32_pt_t()) {
            window_sz = {YV12_buf->w, YV12_buf->h};
        } // if no window size, use 'native' size of render-target-texture
        if (!window) {
        ...
        if (!font_setup_done.v) {
            ...
            if (!font) {
                rt_err_sdl(strprintf("Couldn't load %s pt font from %s", str(ptsize).c_str(), font_fn.c_str()).c_str());
            }
    ...
    void disp_win_t::on_lb(int32_t const x, int32_t const y) {
        ...
        i32_pt_t const disp_off{displayrect->x,
                                displayrect->y}; // display window x,y is the offset where the neg_corner of the texure will be drawn.
        i32_pt_t const tex_sz{int32_t(YV12_buf->w),
                              int32_t(YV12_buf->h)}; // the texture is always it is always drawn resized to the window size (regardless of offset)
        uint32_t out_x = 0;
        ...
        for (uint32_t i = 0; i != imgs->size(); ++i) {
            ...
            i32_pt_t const img_nc = u32_to_i32(
                    imgs_buf_nc[i]); // { int32_t(out_x), int32_t(YV12_buf->h) - int32_t(img->sz.d[1]) };
    ...
    void disp_win_t::drain_sdl_events_and_redisplay(void) {
            ...
            switch (event.type) {
                ...
                case SDL_MOUSEBUTTONDOWN:
                    ...
                    if (event.button.button == SDL_BUTTON_RIGHT) {
                        asio->lb_event.set_is_lb();
                        on_lb(event.button.x, event.button.y);
                    }
                    break;
                ...
                case SDL_KEYDOWN:
                    if (1) { // generate/forward keydown event to disp_util parent/user/client
                        ...
                        asio->lb_event.set_is_key(event.key.keysym.sym);
                        on_lb(mx, my);
                    }
        ...
        for (uint32_t i = 0; i != imgs->size(); ++i) {
            ...
            for (vect_anno_t::const_iterator i = annos->begin(); i != annos->end(); ++i) {
                ...
                // ...
                p_SDL_Surface text(
                        TTF_RenderText_Blended_Wrapped(font.get(), i->str.c_str(), color_to_sdl(i->str_color), 800),
                        SDL_FreeSurface);
