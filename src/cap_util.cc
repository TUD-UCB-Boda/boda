...
namespace boda {
    ...
    void capture_t::init_device(void) {
        ...
        if (-1 == xioctl(cap_fd, VIDIOC_S_FMT, &fmt)) {
            rt_err("VIDIOC_S_FMT");
        } // try to set our changes to the format
        ...
        if (fmt.fmt.pix.pixelformat != V4L2_PIX_FMT_YUYV) {
            rt_err(strprintf("requested pixelformat=V4L2_PIX_FMT_YUYV, but got fmt_res=%s",
                             str(fmt.fmt.pix.pixelformat).c_str()));
        }
        u32_pt_t const fmt_res(fmt.fmt.pix.width, fmt.fmt.pix.height);
        if (fmt_res != cap_res) {
            rt_err(strprintf("requested cap_res=%s, but got fmt_res=%s", str(cap_res).c_str(), str(fmt_res).c_str()));
        }
        ...
        assert_st(fmt.fmt.pix.sizeimage ==
                  (fmt.fmt.pix.bytesperline * cap_res.d[1])); // could handle, but not possible/okay?
    ...
    void capture_t::open_device(void) {
        struct stat st;
        if (-1 == stat(cap_dev.exp.c_str(), &st)) {
            rt_err(strprintf("stat failed for '%s': %d, %s\n",
        ...
        if (-1 == cap_fd) {
            rt_err(strprintf("Cannot open '%s': %d, %s\n", cap_dev.in.c_str(), errno, strerror(errno)));
        }
    }
