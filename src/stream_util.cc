...
namespace boda {
    ...
    struct fd_stream_t : public stream_t {
        ...
        void init(string const &boda_parent_addr, bool const &is_worker) {
            ...
            if (bpa_parts.size() != 3) {
                rt_err("boda_parent_addr must consist of three ':' seperated fields, in the form method:to_parent:to_worker"
            ...
            if (method == "fns") {
                ...
                rt_err("unknown boda_parent_addr type " + bpa_parts[0] +
                       ", should be either 'tcp' (tcp connection to host:port), 'fns' (filenames) or 'fds' (open file descriptor integers)");
            }
        }
    ...
    void setopt(int const fd, int const level, int const optname, int const optval = 1) {
        ...
        if (ret != 0) {
            rt_err(strprintf("setsockopt(level=%s,optname=%s) error: %s",
    ...
    struct sock_stream_t : public stream_t {
        ...
        sock_stream_t(string const &boda_parent_addr, bool const &is_worker_) : is_worker(is_worker_), fd(-1),
                                                                                listen_fd(-1) {
            vect_string host_and_port = split(boda_parent_addr, ':');
            assert_st(host_and_port[0] == "tcp"); // should not be here otherwise
            if (host_and_port.size() != 3) {
                rt_err("for the tcp method, boda_parent_addr must consist of three ':' seperated fields,"
        ...
        void bind_and_listen(string const &port) {
            ...
            hints.ai_flags = AI_PASSIVE |
                             AI_ADDRCONFIG; // bind to wildcard, use AI_ADDRCONFIG scheme for filtering out IPv4/IPv6 if not in use

            int const aret = getaddrinfo(0, port.c_str(), &hints, &rp_bind_addrs);
            if (aret != 0) {
                rt_err(strprintf("getaddrinfo with port %s failed: %s", port.c_str(), gai_strerror(aret)));
            }
            ...
            setopt(listen_fd, SOL_SOCKET,
                   SO_REUSEADDR); // 'helps' with avoiding can't-bind-port after crashes, etc ... see SO posts
        ...
        void connect_to_parent(string const &parent_addr) {
            ...
            if (aret != 0) {
                rt_err(strprintf("getaddrinfo(\"%s\") failed: %s", parent_addr.c_str(), gai_strerror(aret)));
            }
        ...
        void write(char const *const &d, size_t const &sz) {
            if (fd == -1) {
                rt_err("socket_stream_t::write(): no open stream (didn't connect-to-worker/wait-for-worker-to-connect?)");
            }
            size_t sz_written = 0;
            assert_st(sz);
            while (sz_written < sz) {
                ...
                if (ret == -1) {
                    if (errno == EINTR) { continue; }
                        ...
                        fd = -1;
                        rt_err(string("socket-write-error: ") + strerror(errno));
                    }
                }
                if (ret == 0) {
                    fd = -1;
                    rt_err("socket-write-error, ret = 0");
                }
            ...
            while (sz_read < sz) {
                ...
                if (ret == -1) {
                    if (errno == EINTR) { continue; }
                        ...
                        fd = -1;
                        rt_err(string("socket-read-error: ") + strerror(errno));
                    }
                }
                if (ret == 0) {
                    fd = -1;
                    rt_err("socket-read-error, ret = 0 (eof?)");
                }
                assert_st(ret > 0); // FIXME: other returns possible? make into rt_err() call?
                sz_read += ret;
            }
