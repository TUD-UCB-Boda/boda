...
namespace boda {
        ...
        // bases=["data_stream_t"], type_id="add-img")
    {
        ...
        uint32_t grey_to_pel_maybe_inv(uint8_t const &gv) {
            return invert_intensity ? grey_to_pel(255 - gv) : grey_to_pel(gv);
        }
        ...
        virtual void data_stream_init(nesi_init_arg_t *const nia) {
            ...
            rgb_levs_filt_rng = 0;
            comp_gamma_scale *= 255.0f / powf(decompand ? 65535.0f : 4096.0f,
                                              comp_gamma); // FIXME: yeah, i got no idea on the +-1s of the bounds here ... depends on stuff?
        }
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            ...
            if (level_adj && (rgb_levs_filt_min == float_const_min)) { // init level filt on first frame
                data_block_to_img_inner(
                        db); // note: writes (maybe-garbage) values to frame_buf which are unused, but sets frame levels
        ...
        bool maybe_set_per_block_frame_sz(data_block_t const &db) {
            ...
            if (!db.nda) {
                rt_err("data_to_img_raw: frame_sz not set, but data block didn't have nda_dims set. can't determine image dims.");
            }
            ...
            if ((db.nda->dims.tsz() * chans) != get_bytes_per_pel()) {
                rt_err(strprintf(
                        "nda dims / pel format byte size mismatch: db.nda_dims.tsz()=%s but get_bytes_per_pel()=%s\n",
            ...
            if (0) {
                ...
                for (uint32_t y = 0; y < img_sz.d[1]; y += 2) {
                    ...
                    for (uint32_t x = 0; x < img_sz.d[0]; x += 2) {
                        ...
                        if (level_adj) {
                            rt_err("level_adj for uint8_t src pixel data not sensible, refusing to try.");
                        }
                        uint32_t const pel = rgba_to_pel(rgb[0], rgb[1], rgb[2]);
                        dest_data[x] = pel;
                        dest_data[x + 1] = pel;
                        dest_data_yp1[x] = pel;
                        dest_data_yp1[x + 1] = pel;
                    }
                }
                    ...
                    for (uint32_t x = 0; x < img_sz.d[0]; x += 2) {
                        ...
                        if (level_adj) {
                            for (uint32_t d = 0; d != 3; ++d) {
                                ...
                                rgb[d] = clamp((float(rgb[d]) - rgb_levs_filt_min) * (float(uint8_t_const_max) + 1.0f) /
                                               rgb_levs_filt_rng, 0.0f, float(uint8_t_const_max));
                            }
                ...
                for (uint32_t y = 0; y < img_sz.d[1]; ++y) {
                    ...
                    for (uint32_t x = 0; x < img_sz.d[0]; ++x) {
                        ...
                        if (level_adj) {
                            ...
                            gv = clamp((float(gv) - rgb_levs_filt_min) * (float(uint8_t_const_max) + 1.0f) /
                                       rgb_levs_filt_rng, 0.0f, float(uint8_t_const_max));
                ...
                for (uint32_t y = 0; y < img_sz.d[1]; ++y) {
                    ...
                    uint8_t const *const src_data =
                            (uint8_t const *) (db.d()) + src_y * cur_frame_sz.d[0] * get_bytes_per_pel();
                    uint32_t *const dest_data = frame_buf->get_row_addr(y);
                    for (uint32_t x = 0; x < img_sz.d[0]; ++x) {
                    ...
                    for (uint32_t x = 0; x < img_sz.d[0]; ++x) {
                        ...
                        if (level_adj) {
                            ...
                            if (level_adj_log_for_float_data) {
                                gv = logf(gv - rgb_levs_filt_min + 1.0f) * (float(uint8_t_const_max) + 1.0f) /
                                     logf(rgb_levs_filt_rng + 1.0f);
                            } else {
                                gv = (gv - rgb_levs_filt_min) * (float(uint8_t_const_max) + 1.0f) / rgb_levs_filt_rng;
                            }
        ...
        virtual string get_pos_info_str(void) {
            return strprintf(
                    "data-to-img: filter state: rgb_levs_filt_rng=%s rgb_levs_filt_min=%s rgb_levs_frame_min=%s rgb_levs_frame_max=%s",
                    str(rgb_levs_filt_rng).c_str(), str(rgb_levs_filt_min).c_str(), str(rgb_levs_frame_min).c_str(),
                    str(rgb_levs_frame_max).c_str());
        }
