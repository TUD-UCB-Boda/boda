...
namespace boda {
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            ...
            while (1) {
                ...
                if (!mfsr.can_read(sizeof(maybe_magic) + sizeof(lrec))) {
                    rt_err(strprintf(
                            "data_stream_mxnet_brick_t: not at eof, but not enough bytes left in stream to read next record header: "
                                    "bytes_left=%s", str(mfsr.bytes_left()).c_str()));

                }
                mfsr.read_val(maybe_magic);
                if (maybe_magic != mxnet_brick_magic) {
                    rt_err(strprintf("data_stream_mxnet_brick_t: expected magic uint32_t of %x, but got %x",
                                     mxnet_brick_magic, maybe_magic));
                }
                mfsr.read_val(lrec);
                ...
                if ((cflag == 0) || (cflag == 1)) {
                    if (parts.size() != 1) {
                        rt_err(strprintf(
                                "error in mxnet brick stream, expected cflag == 2 or 3 in continutation of split record, but saw cflag=%s\n",
                    ...
                    if (parts.size() == 1) {
                        rt_err(strprintf(
                                "error in mxnet brick stream, expected cflag == 0 or 1 at rec start, saw cflag=%s\n",
                                str(uint32_t(cflag)).c_str()));
                    }
            ...
            if (parts.size() == 1) {
                ...
                for (vect_p_nda_t::const_iterator i = parts.begin(); i != parts.end(); ++i) {
                    if (i != parts.begin()) { // join parts with magic val
                        std::copy((uint8_t const *) &mxnet_brick_magic,
                                  ((uint8_t const *) &mxnet_brick_magic) + sizeof(mxnet_brick_magic),
                                  (uint8_t *) ret.d() + pos);
                        pos += sizeof(mxnet_brick_magic);
                    }
                    std::copy((uint8_t *) (*i)->rp_elems(), (uint8_t *) (*i)->rp_elems() + (*i)->dims.bytes_sz(),
                              (uint8_t *) ret.d() + pos);
                    pos += (*i)->dims.bytes_sz();
                }
        ...
        // bases=["data_stream_t"],type_id="mxnet-brick-sink")
    {
        ...
        virtual string get_pos_info_str(void) {
            return string("data_sink_mxnet_brick: wrote <NOT_IMPL> records in <NOT_IMPL> bytes");
        }
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            if (!(db.sz() < mxnet_brick_max_rec_sz)) {
                rt_err(strprintf("mxnet_brick_max_rec_sz=%s but db.sz=%s",
        ...
        // bases=["data_stream_t"], type_id="test-gen")
    {
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            ...
            if (!(tot_num_gen & 1)) {
                ...
                std::copy((uint8_t const *) &block_hash, ((uint8_t const *) &block_hash) + sizeof(block_hash),
                          (uint8_t *) ret.d());
            }
            ret.frame_ix = tot_num_gen;
            ret.timestamp_ns = tot_num_gen;
        ...
        // bases=["data_stream_t"],type_id="hash-check")
    {
        ...
        virtual string get_pos_info_str(void) {
            return strprintf("data_sink_hash_check: tot_num_read=%s\n", str(tot_num_read).c_str());
        }
