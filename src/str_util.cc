...
namespace boda {
    ...
    string hex(std::string const &s) {
        string ret;
        boost::algorithm::hex(s.begin(), s.end(), std::back_inserter(ret));
        return ret;
    }

    string unhex(std::string const &s) {
        string ret;
        boost::algorithm::unhex(s.begin(), s.end(), std::back_inserter(ret));
        return ret;
    }
    ...
    string pp_fpspw(double const &v, bool const &verbose) {
        return pp_val(v) + (verbose ? " FLOPS/SEC/WATT" : "F/s/W");
    }
