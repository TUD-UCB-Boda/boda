...
namespace boda {
    ...
    inline void bwrite(STREAM &out, pcap_hdr_t const &o) {
    ...
    template<typename STREAM>
    inline void bread(STREAM &in, pcap_hdr_t &o) {
    ...
    template<typename STREAM>
    inline void bwrite(STREAM &out, pcaprec_hdr_t const &o) {
        bwrite(out, o.ts_sec);
    ...
    template<typename STREAM>
    inline void bread(STREAM &in, pcaprec_hdr_t &o) {
        bread(in, o.ts_sec);
        bread(in, o.ts_usec);
        bread(in, o.incl_len);
        bread(in, o.orig_len);
        ...
        // bases=["data_stream_file_t"], type_id="pcap-src")
    {
        ...
        void pcap_src_init(void) {
            bread(mfsr, hdr);
            if (hdr.magic_number != pcap_file_magic) {
                printf("error reading pcap file; expected pcap_file_magic=%s, got hdr.magic_number=%s\n",
            ...
            printf("PCAP header: version_major=%s version_minor=%s thiszone=%s sigfigs=%s snaplen=%s network=%s\n",
                   str(hdr.version_major).c_str(), str(hdr.version_minor).c_str(), str(hdr.thiszone).c_str(),
                   str(hdr.sigfigs).c_str(),
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            ...
            while (1) {
                ...
                if (extract_udp_payload) {
                    ethernet_header const *eth_hdr = (ethernet_header const *) mfsr.consume_borrowed_raw_block(
                            sizeof(ethernet_header)).get();
                    uint16_t const ethertype = ntohs(eth_hdr->ethertype);
                    if (ethertype != ethertype_ipv4) {
                        rt_err(strprintf("expected IPv4 ethertype but got ethertype=%s\n", str(ethertype).c_str()));
                    }
                    ip const *ip_hdr = (ip const *) mfsr.consume_borrowed_raw_block(
                            sizeof(ip)).get(); // with no options
                    if (ip_hdr->ip_v != 4) {
                        rt_err(strprintf("error, expected IPv4 packet, but ip_hdr->ip_v=%s\n",
                                         str(ip_hdr->ip_v).c_str()));
                    }
                    int32_t opts_sz = (int32_t(ip_hdr->ip_hl) << 2) - sizeof(ip);
                    if (opts_sz < 0) {
                        rt_err(strprintf("error, bad IPv4 packet length ip_hdr->ip_hl=%s\n",
                    ...
                    if (udp_payload_sz < 0) {
                        rt_err(strprintf("error, bad UDP packet length ntohs(upd_hdr.len)=%s\n",
                                         str(ntohs(udp_hdr->len)).c_str()));
                    }
                    uint32_t expected_packet_size =
                            sizeof(ethernet_header) + sizeof(ip) + opts_sz + sizeof(udphdr) + udp_payload_sz;
                    if (rec_hdr.incl_len != expected_packet_size) {
                        rt_err(strprintf(
                                "rec_hdr.incl_len=%s rec_hdr.orig_len=%s, but expected_packet_size=%s (if == orig_len but incl_len is smaller, packet was truncated in capture. otherwise, if orig_len == incl_len, the stream is correct/unsupported)\n",
                                str(rec_hdr.incl_len).c_str(), str(rec_hdr.orig_len).c_str(),
                                str(expected_packet_size).c_str()));
                    }
        ...
        // bases=["data_sink_file_t"], type_id="pcap-sink")
    {
        ...
        virtual string get_pos_info_str(void) {
            return string("data_sink_pcal: wrote <NOT_IMPL> packets to:") + data_sink_file_t::get_pos_info_str();
        }
