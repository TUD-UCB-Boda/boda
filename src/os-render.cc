...
void main(){
        ...
        // bases=["data_stream_t"], type_id="add-img-pts")
    {
            ...
            if (opt.name == "camera-pos-rot") {
                ...
                if (print_camera) {
                    printf("cam_pos[=0]%s cam_pos[1]=%s cam_pos[2]=%s\n", str(cam_pos[0]).c_str(),
                           str(cam_pos[1]).c_str(), str(cam_pos[2]).c_str());
                    printf("cam_rot[0]=%s cam_rot[1]=%s cam_rot[2]=%s\n", str(cam_rot[0]).c_str(),
                           str(cam_rot[1]).c_str(), str(cam_rot[2]).c_str());
                }
            }
        ...
        void draw_grid(void) {
            ...
            glDrawArrays(GL_LINES, 0, grid_pts->elems_sz() / 3);
        ...
        void draw_raw_cloud(data_block_t const &db) {
            ...
            if (pt_marker_mode == 0) {
                ...
                p_nda_float_t box_pts = make_shared<nda_float_t>(
                        dims_t{{raw_cloud_pts_sz, 2, 2, 2, inner_dim_sz}, {"pt", "hv", "pn", "be", "d"}, "float"});
        ...
        void init_raw_cloud(void) {
            raw_cloud_programID = LoadShaders(verbose, *read_whole_fn(raw_cloud_vertex_shader_fn),
        ...
        void bind_laser_corrs(void) {
            ...
            if (pcdm_mode.v) {
                for (vect_laser_corr_t::iterator i = laser_corrs_to_bind.begin();
                     i != laser_corrs_to_bind.end(); ++i) { (*i).rot_corr = 0.0; }
            ...
            glBufferData(GL_TEXTURE_BUFFER, laser_corrs_to_bind.size() * sizeof(laser_corrs_to_bind[0]),
                         laser_corrs_to_bind.data(), GL_STATIC_DRAW);
            glBindBuffer(GL_TEXTURE_BUFFER, 0);
...
//      ctx = OSMesaCreateContextExt( OSMESA_RGBA, 16, 0, 0, NULL );
            ...
            if (!OSMesaMakeCurrent(ctx, frame_buf->get_row_addr(0), GL_UNSIGNED_BYTE, frame_buf->sz.d[0],
                                   frame_buf->sz.d[1])) {
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            ...
            if (endswith(db.meta, "/PCDM")) {
                pcdm_mode.v = 1;
                bind_laser_corrs();
            } // permanently set. rebind here since we might not bind again later.
            p_nda_t azi_nda;
                ...
                for (uint32_t i = 0; i != db.subblocks->size(); ++i) {
                    ...
                    if (sdb.meta == "lidar-corrections") {
                        ...
                        rt_err(strprintf("os-render: unknown subblock with meta=%s tag=%s\n", str(sdb.meta).c_str(),
                                         str(sdb.tag).c_str())); // could maybe just skip/ignore
                    }
                }
            }
            if (db.nda) {
        ...
        virtual void new_frame_setup(void) {
            ...
            glm::mat4 Projection = glm::perspective(glm::radians(60.0f),
                                                    float(frame_buf->sz.d[0]) / float(frame_buf->sz.d[1]), 0.1f,
                                                    1000.0f);
            // Or, for an ortho camera :
            //glm::mat4 Projection = glm::ortho(-10.0f,10.0f,-10.0f,10.0f,0.0f,100.0f); // In world coordinates
#if 1
            glm::mat4 R = glm::rotate(glm::radians(180.0f) - cam_rot[0], glm::vec3(0.0f, 0.0f, 1.0f)) *
                          glm::rotate(-cam_rot[1], glm::vec3(1.0f, 0.0f, 0.0f));
