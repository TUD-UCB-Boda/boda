...
namespace boda {
    ...
    template<typename T>
    void nda_T<T>::check_tn(void) const {
        string const &type_tn = get_ndat<T>().tn; // FIXME: can the user trigger this error? use better err msg?
        if (dims.tn != type_tn) {
            rt_err(strprintf("internal error: nda_T check_tn() failure: type_tn=%s dims.tn=%s\n",
    ...
    template
    class nda_T<uint16_t>;

    template
    class nda_T<uint32_t>;
    ...
    void nda_t::reshape(dims_t const &new_dims) {
        ...
        if (dims.bytes_sz() != new_dims.bytes_sz()) {
            rt_err(strprintf(
                    "nda_t::reshape(): new_dims=%s incompatible with existing dims=%s: different size in bytes.",
    ...
    void neg_one_fail(int const &ret, char const *const func_name) {
        if (ret == -1) {
            rt_err(strprintf("%s() failed with errno=%s (%s)", func_name, str(errno).c_str(),
                             strerror(errno)));
        }
    }

    void rt_err_errno(char const *const func_name) {
        rt_err(strprintf("%s failed with errno=%s (%s)", func_name, str(errno).c_str(),
                         strerror(errno)));
    }
    ...
    template<typename T>
    struct ssds_diff_per_type_T : public ssds_diff_per_type_t {
        ssds_diff_t &v;
        T *o1;
        T *o2;
        ...
        virtual void cnt_diff_elems(void) {
            for (uint64_t i = 0; i < v.sz; ++i) {
                if (o1[i] != o2[i]) { ++v.num_diff; }
            }
        }
        ...
        virtual double get_rd_double(dims_iter_t const &di) {
            return min_sig_mag_rel_diff(1.0, nda_at<T>(v.o1, di), nda_at<T>(v.o2, di));
        }
    ...
    struct ndd_si_t {
        uint64_t stride;
        uint64_t offset;
        uint64_t num_subsamps;
    }; // nda_digest_data sample-info struct
    typedef vector<ndd_si_t> vect_ndd_si_t;
    typedef shared_ptr<vect_ndd_si_t> p_vect_ndd_si_t;

    template<typename T>
    struct nda_digest_T : public nda_digest_t {
        dims_t dims;
        uint64_t seed;
        T min_v;
        T max_v;
        ...
        virtual void set_from_nda(p_nda_t const &nda, uint64_t const &seed_) {
            ...
            for (vect_ndd_si_t::const_iterator i = sis->begin(); i != sis->end(); ++i) {
                samps.push_back(get_samp(ve, sz, *i));
            }
        }
    ...
    template<typename T>
    string mrd_comp(nda_digest_T<T> const &v1, nda_digest_T<T> const &v2, double const &mrd) {
        if (v1.dims != v2.dims) {
            return strprintf("nda_digest dims mismatch: v1.dims=%s v2.dims=%s",
                             str(v1.dims).c_str(), str(v2.dims).c_str());
        }
        if (v1.seed != v2.seed) {
            return strprintf("nda_digest seed mismatch: v1.seed=%s v2.seed=%s",
        ...
        for (uint32_t i = 0; i != v1.samps.size(); ++i) {
            ...
            string const tag = strprintf("stride=%s,offset=%s", str(si.stride).c_str(),
                                         str(si.offset).c_str()); // FIXME defer
    ...
    template<typename T>
    string nda_digest_T<T>::mrd_comp(p_nda_digest_t const &o, double const &mrd) {
    ...
    template<typename T>
    std::ostream &operator<<(std::ostream &out, nda_digest_T<T> const &o) {
        ...
        for (uint32_t i = 0; i != o.samps.size(); ++i) {
            out << strprintf(" (s=%s,o==%s)=%s", str(sis->at(i).stride).c_str(), str(sis->at(i).offset).c_str(),
                             str(o.samps[i]).c_str());
        }
        return out;
    }
    ...
    inline void bwrite(STREAM &out, nda_digest_T<T> const &o) {
        bwrite(out, ndd_ver1);
    ...
    template<typename STREAM, typename T>
    inline void bread(STREAM &in, nda_digest_T<T> &o) {
        uint32_t magic_ver;
        bread(in, magic_ver);
    ...
    template<typename STREAM, typename P, template<typename> class PT>
    struct bread_derived_t {
        STREAM &in;
        P &v;
    ...
    template<typename T>
    void nda_digest_T<T>::bwrite(std::ostream &out) const {
        boda::bwrite(out, dims.tn);
        boda::bwrite(out, *this);
    }

    template<typename P, template<typename> class PT>
    struct make_derived_t {
        P &v;

        make_derived_t(P &v_) : v(v_) {}

        template<typename T>
        void operator()(void) const { v = std::make_shared<PT<T> >(); }
    };
    ...
    struct nda_c_const_str_t {
        ...
        void op(nda_t const &nda) const {
            ...
            for (uint32_t i = 0; i != nda.elems_sz(); ++i) {
                if (i) { out << ":"; }
                elem_c_const_str(elems[i]);
            }
        }

        template<typename T>
        void elem_c_const_str(T const &v) const { out << v; }
    ...
    struct nda_lt_elems_t {
        ...
        void op(nda_t const &o) const {
            ...
            for (uint32_t i = 0; i != t.elems_sz(); ++i) {
                if (te[i] < oe[i]) {
                    ret = 1;
                    return;
                }
    ...
    bool nda_t::operator<(nda_t const &o) const {
        ...
        if (!bool(d)) {
            assert_st(!bool(o.d));
            return 0;
        } // in fact, *this and o are equal here
        bool ret = 0;
    ...
    struct uint8_t_munmap_deleter {
        ...
        void operator()(uint8_t *const &b) const {
            if (!length) { return; }
            if (munmap(b, length) == -1) { rt_err("munmap"); }
        }
    };
    ...
    void *posix_memalign_check(size_t const sz, uint32_t const a) {
        ...
        if (ret) {
            rt_err(strprintf("posix_memalign( p, %s, %s ) failed, ret=%s",
    ...
    filename_t ensure_one_is_regular_file(filename_t const &fna, filename_t const &fnb) {
        if (is_regular_file(path(fna.exp))) { return fna; }
            ...
            rt_err(strprintf(
                    "neither file '%s' (expanded: '%s') nor alternate file '%s' (expanded: '%s') is a regular file",
    ...
    bool ifs_getline(std::string const &fn, p_istream const &in, string &line) {
        ...
        if (in->eof()) {
            if (!line.empty()) {
                rt_err((fn.empty() ? string() : ("reading " + fn + ": ")) + "incomplete (no newline) line at EOF:'" +
                       line + "'");
            }
            return 1;
        } else {
    ...
    p_mapped_file_source map_file_ro(filename_t const &fn) {
        ...
        try { ret.reset(new mapped_file_source(fn.exp)); }
            ...
            rt_err(strprintf("failed to open/map file '%s' (expanded: '%s') for reading", fn.in.c_str(),
                             fn.exp.c_str()));
        }
    ...
    void write_whole_fn(std::string const &fn, std::string const &data) {
        return write_whole_fn(filename_t{fn, fn}, data);
    }
