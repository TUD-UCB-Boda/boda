...
namespace boda {
    ...
    void run_xpose(p_op_base_t const &anno_op, rtc_codegen_t &codegen, string const &xpose_func_name,
        ...
        p_rcg_func_call_t rfc = codegen.gen_func_override_func_name(xpose_func_name, *anno_op,
                                                                    map_str_rtc_arg_t{{out_an_and_vn, out_an_and_vn},
                                                                                      {in_an_and_vn,  in_an_and_vn}});
    ...
    struct rtc_var_holder_t {
        ...
        void create(string const &vn, dims_t const &dims) {
            vns.push_back(vn);
            return codegen.rtc->create_var_with_dims(vn, dims);
        }
    ...
    prc_ret_t profile_rcg_call(p_op_base_t const &anno_op, rtc_codegen_t &codegen,
        ...
        p_rcg_func_call_t rfc = codegen.gen_func(*anno_op,
                                                 map_str_rtc_arg_t()); // FIXME: not passing in args here. yet?
        ...
        if (in_gen_op_orig) {
            for (vect_arg_decl_t::multi_iter i = rcg->rtc_func_template->arg_decls.multi_begin(
                ...
                //if( i.vn() == "cucl_arg_info" ) { continue; } // FIXME: not-too-nice special case for cucl_arg_info argument
                if (i.ad().loi.v ==
                    0) { continue; } // FIXME: not-too-nice special case for scalars ... better be const.
                ...
                dims_t const &ref_in_dims = anno_op->has(ref_in_dims_name) ? anno_op->get_dims(ref_in_dims_name)
                                                                           : in_dims;
                ...
                if (gen_vn != i.vn()) {
                    ...
                    // FIXME: sigh.
                    if ((i.vn() == "filts") &&
                        is_k1_or_t_or_reg_conv(anno_op->get_func_name())) { xpose_op = "xpose_filts"; }
        ...
        for (vect_arg_decl_t::multi_iter i = rcg->rtc_func_template->arg_decls.multi_begin(
            ...
            dims_t const &ref_out_dims = anno_op->has(ref_out_dims_name) ? anno_op->get_dims(ref_out_dims_name)
                                                                         : out_dims;
            string gen_vn = i.vn();
    ...
    p_conv_op_base_t
    make_p_conv_op_base_t_init_and_check_unused_from_lexp(p_lexp_t const &lexp, nesi_init_arg_t *const nia);
    ...
    p_rtc_compute_t
    make_p_rtc_compute_t_init_and_check_unused_from_lexp(p_lexp_t const &lexp, nesi_init_arg_t *const nia);
    ...
    void ops_prof_t::main(nesi_init_arg_t *nia) {
        ...
        if (wisdom_in_fn) {
            ...
            if (boost::filesystem::is_regular_file(wisdom_in_fn->exp)) { win = ifs_open(*wisdom_in_fn); }
                ...
                (*out)
                        << "warning: specified input wisdom file could not be loaded; ignoring. known-good testing disabled.\n";
            } //
        }

        // by default, add all enabled/availible backends
        if (rtcs.size() != rtcns.size()) {
            rt_err(strprintf("must specific the same # of rtcs and rtcns, but rtcs.size()=%s and rtcns.size()=%s\n",
                             str(rtcs.size()).c_str(), str(rtcns.size()).c_str()));
        }
        if (rtcs.empty()) {
            if (is_feature_enabled("nvrtc")) {
                ops_be_t ops_be{"nvrtc",
                                make_p_rtc_compute_t_init_and_check_unused_from_lexp(parse_lexp("(be=nvrtc)"), nia)};
                must_insert(ops_bes, ops_be.rtcn, ops_be);
            }
            if (is_feature_enabled("opencl")) {
                ops_be_t ops_be{"ocl",
                                make_p_rtc_compute_t_init_and_check_unused_from_lexp(parse_lexp("(be=ocl)"), nia)};
                must_insert(ops_bes, ops_be.rtcn, ops_be);
            }
        ...
        for (uint32_t op_ix = 0; !ifs_getline(ops_fn.exp, ops,
                                              line); op_ix++) { // op_ix is for printouts, helpful to run specific op later
            ...
            if (skip_ops) {
                --skip_ops;
                continue;
            } // note: we already read from op_wisdom_in ... probably what we want?
            if (op_wisdom_in && (*op_wisdom_in->op != *op_wisdom_out->op)) {
                rt_err(strprintf(
                        "op mismatch between input wisdom and ops-list (to output): op_wisdom_in->op=%s op_wisdom_out=%s",
            ...
            vect_uint32_t out_wixs_to_run; // will contain all wixs specified on the command line, with the known-good wix first
            {
                ...
                for (map_str_op_tune_t::const_iterator i = op_tunes.begin(); i != op_tunes.end(); ++i, ++wix) {
                    op_wisdom_out->wisdoms.push_back(
                            p_op_tune_wisdom_t(new op_tune_wisdom_t{make_shared<op_tune_t>(i->second)}));
            ...
            for (vect_uint32_t::const_iterator wix = out_wixs_to_run.begin(); wix != out_wixs_to_run.end(); ++wix) {
                ...
                if ((*wix) ==
                    kg_wix) { // if this is the to-use-as-known-good op_tune, store it's results in vs1, and maybe write its digest.
                    assert_st(wix == out_wixs_to_run.begin()); // should be first run
                    if (!err.str().empty()) {
                        err << strprintf(
                                "known-good op_tune (kg_tune_tag=%s) failed. Can't write digests or do live comparisons.",
                                kg_tune_tag.c_str());
                    } else {
                        vs_kg = vsi;
                        if (write_kg_digest) {
                            for (map_str_p_nda_t::const_iterator i = vs_kg->begin(); i != vs_kg->end(); ++i) {
                                size_t const digest_seed = std::hash<string>()(
                                        i->first); // FIXME: make better seed by including op/op_tune/???
                                op_wisdom_out->kgs.push_back(pair_str_p_nda_digest_t(i->first,
                                                                                     nda_digest_t::make_from_nda(
                                                                                             i->second, digest_seed)));
                            }
                        }
                    }
                }
                ...
                if (err.str().empty()) { // if tune ran without error, do compares
                    ...
                    if ((prc_ret.op->get_func_name() == "cudnn_conv") &&
                        (get_xy_dims(op_wisdom_out->op->get_dims("kern_sz")) == u32_pt_t{3, 3})) {
                        vmt = 2e-3;
                    }
                    // full-data compare
                    ...
                    if (op_wisdom_in) {
                        ...
                        for (map_str_p_nda_t::const_iterator i = vsi->begin(); i != vsi->end(); ++i, ++vix) {
                            ...
                            if (!comp_res.empty()) {
                                err << (i->first) + " digest mrd_comp() failure vs stored digest in '" +
                                       wisdom_in_fn->in + "'";
                                err_extra << "comp_res:\n" + comp_res + "\n";
                            }
                        }
    ...
    void gen_ops_prof_tests(p_ostream &out, bool const &run_slow) {
        ...
        if (run_slow) {
            run_bases_conv.push_back({"conv-full-gen5",
                                      cli_base + " --ops-fn='%(boda_test_dir)/conv-ops-1-5-20-nin-alex-gn.txt'" +
                                      gen_data_mode_5});
        }
        run_bases_conv.push_back(
                {"conv-debug", cli_base + " --ops-fn='%(boda_test_dir)/conv-ops-debug-tmp.txt'" + gen_data_mode_5});
        (*out) << "<root>\n";
