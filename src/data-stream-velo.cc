...
namespace boda {
    ...
    std::ostream &operator<<(std::ostream &os, laser_corr_t const &v) {
        os << strprintf(
                "vert_corr=%s rot_corr=%s dist_corr=%s dist_corr_x=%s dist_corr_y=%s off_corr_vert=%s off_corr_horiz=%s focal_dist=%s focal_slope=%s",
                str(v.vert_corr).c_str(), str(v.rot_corr).c_str(), str(v.dist_corr).c_str(), str(v.dist_corr_x).c_str(),
                str(v.dist_corr_y).c_str(), str(v.off_corr_vert).c_str(), str(v.off_corr_horiz).c_str(),
                str(v.focal_dist).c_str(), str(v.focal_slope).c_str());
        return os;
    }
        ...
        // bases=["has_main_t"], type_id="test-velo-crc")
    {
        ...
        void main(nesi_init_arg_t *nia) {
            ...
            if (expected_crc_str.size() != 2) {
                rt_err(strprintf("hex_crc=%s must unhex into exactly 2 bytes\n", str(hex_crc).c_str()));
            }
        ...
        // bases=["data_stream_t"], type_id="velodyne")
    {
        ...
        virtual string get_pos_info_str(void) {
            return strprintf("tot_num_read=%s vps info:%s",
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            ...
            while (!ret_db.valid()) {
                ...
                if (verbose > 10) {
                    printf("  packet: si->gps_timestamp_us=%s si->status_type=%s si->status_val=%s\n",
                           str(si->gps_timestamp_us).c_str(), str(si->status_type).c_str(),
                           str(uint16_t(si->status_val)).c_str());
                }
                for (uint32_t fbix = 0; fbix != fbs_per_packet; ++fbix) {
                    ...
                    if (tot_lasers == 64) {
                        if (bi->block_id != laser_block_ids[fbix & 1]) {
                            rt_err(strprintf(
                                    "(64 laser mode) saw unexpected bi->block_id=%s for firing block fbix=%s\n",
                        ...
                        if (bi->block_id != laser_block_ids[0]) {
                            rt_err(strprintf(
                                    "(32 laser mode) saw unexpected bi->block_id=%s for firing block fbix=%s\n",
                                    str(bi->block_id).c_str(), str(fbix).c_str()));
                        }
                    } else { assert_st(0); }
                    ...
                    if ((tot_lasers == 64) && dual_return_and_use_only_first_return) {
                        if (fbix & 2) { // skip second return blocks, but check that they are the same rot
                            if (bi->rot_pos != last_rot) {
                                rt_err(strprintf(
                                        "error skipping second return block: expected bi->rot_pos=%s to equal processed block rot last_rot=%s."
                                                " refusing to proceed.",
                    ...
                    if (tot_lasers == 64) {
                        if (!(fbix & 1)) {
                            last_ub_rot = bi->rot_pos;
                            continue;
                        } // FIXME: handle upper/lower more cleanly
                        else {
                            if (bi->rot_pos != last_ub_rot) {
                                rt_err(strprintf(
                                        "error on second block for 64 laser sensor: expected bi->rot_pos=%s to equal last_ub_rot=%s."
                                                " refusing to proceed.",
                    ...
                    if (rots_till_emit == uint32_t_const_max) { // if not triggered yet
                        if ((last_rot != uint16_t_const_max) &&
                            rel_angle_lt(last_rot, fov_center_rot) &&
                            !rel_angle_lt(bi->rot_pos, fov_center_rot)) { // trigger
                            ret_db.timestamp_ns = vps_db.timestamp_ns;
                            if (verbose > 4) { printf("-- TRIGGER -- bi->rot_pos=%s\n", str(bi->rot_pos).c_str()); }
                            if (verbose > 4) {
                                printf("  @TRIGGER: si->gps_timestamp_us=%s si->status_type=%s si->status_val=%s\n",
                                       str(si->gps_timestamp_us).c_str(), str(si->status_type).c_str(),
                                       str(uint16_t(si->status_val)).c_str());
                            }
                        ...
                        if (!rots_till_emit) {
                            ...
                            for (uint32_t j = 0; j != tot_lasers; ++j) {
                                for (uint32_t i = 0; i != fov_rot_samps; ++i) {
                                    ...
                                    if (!j) {
                                        out_nda_azi->at1(i) = buf_nda_azi->at1(buf_rot);
                                    } // just do once per rot (i.e. sorta-should be outside j loop)
                                }
                            }
                    ...
                    if (last_rot != uint16_t_const_max) {
                        if (last_rot == bi->rot_pos) {
                            ...
                            if (verbose > 50) {
                                float azi_step =
                                        float(rel_angle_delta(bi->rot_pos, last_rot)) / 100.0; // FIXME: smooth?
                                printf("velo block: fbix=%s azi_step=%s packet_ix=%s\n", str(fbix).c_str(),
                                       str(azi_step).c_str(), str(packet_ix).c_str());
                            }
                        }
                    }
                    last_rot = bi->rot_pos;
                    buf_nda_rot += 1;
                    if (buf_nda_rot == fov_rot_samps) { buf_nda_rot = 0; } // FIXME: don't we have inc_mod for this?
                }
            }
            ret_db.meta = "lidar";
        ...
        status_info_t const &read_status_epoch(uint32_t const &laser, uint32_t const &offset) {
            ...
            uint32_t const six = velo_packets_in_cycle * ((4 * laser) + (offset / 7)) +
                                 (velo_cycle_prefix_types.size() + (offset % 7));
            assert_st(six <
                      status_ring.size()); // by constuction and earlier asserts, must hold even for any invalid stream.
            return status_ring[six];
        }

        uint8_t read_status_epoch_uint8_t(uint32_t const &laser, uint32_t const &offset) {
            return read_status_epoch(laser, offset).status_val;
        }

        uint16_t read_status_epoch_uint16_t(uint32_t const &laser, uint32_t const &offset) {
            return uint16_t(read_status_epoch_uint8_t(laser, offset)) +
                   (uint16_t(read_status_epoch_uint8_t(laser, offset + 1)) << 8);
        }
        ...
        void proc_status_epoch(void) {
            if (!status_ring.full()) {
                on_bad_status(
                        "velodyne stream corrupt; should be at end of epoch, but didn't see enough status data since last sync'd point.");
                return;
            }
            ...
            if (print_status_epoch) {
                ...
                printstr(status_msg("cycle 260, offset 11 (config byte slot '3')",
                                    status_ring.at(259 * 16 + velo_cycle_prefix_types.size() + 2)));
                printstr(status_msg("cycle 260, offset 12 (config byte slot '4')",
                                    status_ring.at(259 * 16 + velo_cycle_prefix_types.size() + 3)));
                printstr(status_msg("cycle 260, offset 13 (config byte slot '5')",
                                    status_ring.at(259 * 16 + velo_cycle_prefix_types.size() + 4)));
#endif
            }
            ...
            for (uint32_t i = 0; i != 64; ++i) {
                for (uint32_t j = 0; j != 21; ++j) {
                    real_config_data.push_back(status_ring.at(
                            (i * 4 + 1 + (j / 7)) * 16 + velo_cycle_prefix_types.size() + (j % 7)).status_val);
                }
            }
            ...
            if (!has_checksum) {
                if (len_or_cs != 1820) {
                    on_bad_status(
                            "velodyne stream corrupt or unsupported; assumed HDL64-S2 stream (with no checksum) had non-1820 config-data-length of " +
                            str(len_or_cs));
                    return;
                }
            } else {
            ...
            for (uint32_t i = 0; i != laser_corrs.size(); ++i) {
                uint32_t pos = 21 * i;
                uint8_t const lid = real_config_data.at(pos);
                ++pos;
                if (lid != i) {
                    on_bad_status(strprintf("velodyne config corrupt: expected config for laser %s but saw lid=%s\n",
                                            str(i).c_str(), str(lid).c_str()));
                    return;
                }
            ...
            std::copy((uint8_t *) &laser_corrs[0], (uint8_t *) &laser_corrs[0] + laser_corrs_nda->dims.bytes_sz(),
                      (uint8_t *) laser_corrs_nda->rp_elems());
        }

        void proc_status_cycle(void) {
            assert_st(status_ring.size() >=
                      velo_packets_in_cycle); // 16 (total packets/cycle) = 9 (# prefix statuses) + 7 (per-cycle stuff)
            if (cycle_in_epoch ==
                uint32_t_const_max) { // if we're confused/unsynced, just look for 0xFE as first non-prefix status type
                if (status_ring[status_ring.size() - 7].status_type ==
                    0xFEu) { cycle_in_epoch = 257; } // at last-2 cycle of epoch
            }
        ...
        void proc_status(data_block_t const &db, status_info_t const &si) {
            ...
            if (last_status_gps_ts != uint32_t_const_max) { // if we had a prior timestamp
                ...
                if (si.gps_timestamp_us < last_status_gps_ts) {
                    ...
                    uint32_t max_ts_delta = (tot_lasers == 32) ? 600 : (dual_return_and_use_only_first_return ? 200
                                                                                                              : 300);
                    if (ts_delta > max_ts_delta) {
                        printf("large (>max_ts_delta=%s) ts_delta=%s:\n", str(max_ts_delta).c_str(),
                               str(ts_delta).c_str());
                        had_err = 1;
                    }
                }
                if (had_err) {
                    printf("  @ si.gps_timestamp_us=%s db.timestamp_ns=%s src_pos=%s (prior packet: gps=%s db=%s src_pos=%s);\n",
                           str(si.gps_timestamp_us).c_str(), str(db.timestamp_ns).c_str(),
                           vps->get_pos_info_str().c_str(),
                           str(last_status_gps_ts).c_str(), str(last_status_db_ts).c_str(),
                           last_status_src_pos.c_str());
                }
            }
            ...
            if (packet_in_cycle < velo_cycle_prefix_types.size()) {
                if (si.status_type != velo_cycle_prefix_types[packet_in_cycle]) {
                    on_bad_status(strprintf(
                            "velodyne stream corrupt; at packet_in_cycle=%s, saw status type byte si.status_type=%s but expected velo_cycle_prefix_types[packet_in_cycle]=%s",
                            str(packet_in_cycle).c_str(), str(uint32_t(si.status_type)).c_str(),
                            str(uint32_t(velo_cycle_prefix_types[packet_in_cycle])).c_str()));
                    return;
        ...
        virtual void data_stream_init(nesi_init_arg_t *nia) {
            ...
            if (tot_lasers == 32) {
                if (dual_return_and_use_only_first_return) {
                    rt_err("dual return mode not implemented for 32 laser sensor (doesn't exist?)");
                }
            }
            ...
            if ((fov_center < 0.0) || (fov_center >= 360.0)) {
                rt_err(strprintf("fov_center must be in [0.0,360.0) but was =%s",
            ...
            if (tot_lasers == 32) {
                if (!laser_to_row_ix_str) {
                    ...
                    if (*laser_to_row_ix_str != "default-32") {
                        rt_err("currently, only the 'default-32' laserremapping order for 32 laser sensors is supported");
                    }
                    ...
                    for (uint32_t blix = 0; blix != 16; ++blix) { // most downward first
                        for (uint32_t block = 0; block != 2; ++block) { // lower, upper
                            ...
                            uint32_t const row =
                                    31 - (block * 16 + blix); // row in scanline order (note: 31 - (rest) flips y axis)
                ...
                if (!laser_to_row_ix_str) {
                    ...
                    if (laser_to_row_ix_str_parts.size() != tot_lasers) {
                        rt_err(strprintf(
                                "expected tot_lasers=%s ':' seperated indexes in laser_to_row_ix_str=%s, but got laser_to_row_ix_str_parts.size()=%s\n",
                                str(tot_lasers).c_str(), str(laser_to_row_ix_str).c_str(),
                                str(laser_to_row_ix_str_parts.size()).c_str()));
                    }
                    for (uint32_t i = 0; i != tot_lasers; ++i) {
                        try { laser_to_row_ix.push_back(lc_str_u32(laser_to_row_ix_str_parts[i])); }
                            ...
                            rte.err_msg = "parsing element " + str(i) + " of laser_to_row_ix_str: " + rte.err_msg;
                            throw;
                        }
                    }
                }
            ...
            for (uint32_t i = 0; i != tot_lasers; ++i) {
                if (laser_to_row_ix_sorted[i] != i) {
                    rt_err("the elements of laser_to_row_ix_sorted are not a permutation of [0,tot_lasers)");
                }
            }
    ...
    void read_velo_config(filename_t const &velo_config_xml_fn, vect_laser_corr_t &laser_corrs) {
        ...
        for (xml_node item = points_.child("item"); item; item = item.next_sibling("item")) {
            ...
            if (oid != lix_as_str) {
                rt_err(strprintf("velo config parse failed: expected object_id oid=%s to match lix_as_str=%s\n",
                                 str(oid).c_str(), str(lix_as_str).c_str()));
            } // FIXME: too strong?
  ...
  <item>
        ...
        // bases=["data_stream_t"], type_id="velodyne-gen")
    {
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            ...
            assert_st(
                    !cur_in_nda); // no packet ready should imply no buffered input data remains (but, note there can be a packet ready with no data left).
            cur_in_nda = db.nda;
            cur_in_azi_ix = 0;
            consume_some_input();
        ...
        void consume_some_input(void) {
            ...
            if (cur_in_num_lasers != tot_lasers) {
                rt_err(strprintf(
                        "velodyne-gen: configured to output tot_lasers=%s but got block with cur_in_num_lasers=%s\n",
            ...
            while (cur_out_fb_ix < fbs_per_packet) {
                ...
                if ((cur_azi_deg < 0.0) || (cur_azi_deg >= 360.0)) {
                    rt_err(strprintf(
                            "cur_azi_deg must be in [0.0,360.0) but was =%s -- fov_center bad? azi_step too high? too many azi samples in frame?",
                            str(cur_azi_deg).c_str()));
                }
                ...
                ++cur_in_azi_ix;
                if (cur_in_azi_ix ==
                    num_azis) { // if we exactly finished our current input block, clear it out, and we must return
                    cur_in_azi_ix = uint32_t_const_max;
                    cur_in_nda.reset();
                    return;
                }
        ...
        data_block_t emit_packet(void) {
            ...
            if (cur_timestamp >=
                hour_in_us) { cur_timestamp -= hour_in_us; } // wrap timestamp each hour; note: assumes step+hour_in_us fits in uint32_t
        ...
        // bases=["data_stream_t"], type_id="velo-pcdm-to-xyz")
    {
        ...
        virtual void data_stream_init(
                nesi_init_arg_t *const nia) {} // init is mostly done per-frame (or at least deferred to first frame)
        ...
        void setup_laser_corrs(string const &meta, data_block_t const &db) {
            ...
            if (0) {}
                ...
                if (y_sz != 64) {
                    rt_err(strprintf(
                            "for PCDM or VD_HDL64 lidar data, there must be exactly 64 lines per frame, but saw %s lines.",
                            str(y_sz).c_str()));
                }
                ...
                if (y_sz != 32) {
                    rt_err(strprintf(
                            "for VD_HDL32 lidar data, there must be exactly 64 lines per frame, but saw %s lines.",
                            str(y_sz).c_str()));
                }
            ...
            assert_st(laser_corrs.size() ==
                      64); // for now, in all cases, we should have 64 lasers of corrections here (even in 32 laser case)
            ...
            if (init_no_corrs && is_pcdm) {
                for (vect_laser_corr_t::iterator i = laser_corrs.begin();
                     i != laser_corrs.end(); ++i) { (*i).rot_corr = 0.0; }
        ...
        void setup_azis(data_block_t const &db) {
            ...
            if (!azi_nda) {
                ...
                p_nda_uint16_t azi_nda_u16 = make_shared<nda_uint16_t>(
                        dims_t{dims_t{{fov_rot_samps}, {"x"}, "uint16_t"}});
                for (uint32_t i = 0; i != fov_rot_samps; ++i) {
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            ...
            p_nda_float_t xyz_nda = make_shared<nda_float_t>(
                    dims_t{dims_t{{xy_sz.d[1], xy_sz.d[0], 3}, {"y", "x", "xyz"}, "float"}});
