...
namespace boda {
    ...
    template<>
    void i32_pt_t::read_from_line_parts(vect_string const &parts, uint32_t const init_ix) {
    ...
    template<>
    void i32_box_t::read_from_line_parts(vect_string const &parts, uint32_t const init_ix) {
        p[0].read_from_line_parts(parts, init_ix);
        p[1].read_from_line_parts(parts, init_ix + 2);
    }
    ...
    template<>
    void i32_box_t::from_pascal_coord_adjust(void) {
        for (uint32_t d = 0; d < 2; ++d) {
            --p[0].d[d]; // adjust 1 based [] coords into 0 based [) ones.
            // check for strict normalization
    ...
    template<>
    void i32_box_t::to_pascal_coord_adjust(void) {
        for (uint32_t d = 0; d < 2; ++d) {
            ++p[0].d[d]; // adjust 0 based [) coords into 1 based [] ones.
            assert(p[1].d[d] >= p[0].d[d]); // check for non-strict normalization
        }
    ...
    template<>
    std::string i32_box_t::pascal_str(void) const {
