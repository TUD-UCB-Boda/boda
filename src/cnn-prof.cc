...
namespace boda {
    ...
    void cnn_op_info_t::main(nesi_init_arg_t *nia) {
        ...
        if (rtc_comp) {
            rtc_comp->init();
            codegen_comp.init(rtc_comp, make_cnn_custom_codegen_t(), compile_opts);
        }
        bool const enable_prof = 0;
        if (enable_prof) {
            rtc->profile_start();
            if (rtc_comp) { rtc_comp->profile_start(); }
        }
        ...
        for (vect_string::const_iterator i = in_lines->begin(); i != in_lines->end(); ++i) {
            ...
            if (err.empty() && rtc_comp) {
                ...
                if (vns1 != vns2) {
                    rt_err(strprintf("reg/comp out var set mismatch: vns1=%s vns2=%s\n",
            ...
            if (oet_out) {
                to_latex.eff_row(oet_out.get(), anno_op->get_type(), rfc_dur_secs, peak_flops, rfc_dur_secs_comp);
            }
        ...
        if (enable_prof) {
            rtc->profile_stop();
            if (rtc_comp) { rtc_comp->profile_stop(); }
        }
        rtc->finish_and_sync();
