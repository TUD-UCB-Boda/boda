...
namespace boda {
        ...
        // bases=["has_main_t"], type_id="display-raw-vid")
    {
        ...
        void read_next_block(void) {
            ...
            if (!db.valid()) {
                if (auto_restart) {
                    if (!src->seek_to_block(0)) {
                        printf("auto-restart: seek to db.frame_ix=0 failed. disabling.\n");
                        auto_restart = 0;
                    }
            ...
            if (!db.has_subblocks()) {
                rt_err(strprintf("expected subblocks, but num_subblocks=%s\n", str(db.num_subblocks()).c_str()));
            }
            ...
            for (uint32_t i = 0; i != in_imgs.size(); ++i) {
                ...
                in_imgs[i] = resample_to_size(img,
                                              in_imgs[i]->sz); // may or may not actually downsample, should never up-samp (currently)
        ...
        void on_lb(error_code const &ec) {
            ...
            if (!lbe.is_key) {
                ...
                auto_adv = 0;
            } else if (lbe.is_key && (lbe.keycode == 'r')) {
                if (!src->seek_to_block(0)) {
                    printf("seek to db.frame_ix=0 failed.\n");
                }
            }
