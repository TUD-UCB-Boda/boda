...
namespace boda {
    ...
    void gt_det_t::darknet_to_img_space_conv(u32_pt_t const &img_sz) {
        ...
        rng_check("y", y);
        rng_check("w", w);
        rng_check("h", h); // all input vals should be normalized
        ...
        rng_check("y1", y1);
        rng_check("x2", x2);
        rng_check("y2", y2); // all box edges should be normalized too
...
//#define PASCAL_LAX_PARSE
    ...
    void load_pil_t::load_pascal_data_for_id(string const &img_id, bool load_img, uint32_t const in_file_ix,
                                             bool check_ix_only) {
        ...
        if (check_ix_only) {
            ...
            if (img_info->ix != in_file_ix) {
                rt_err(strprintf("already-loaded image had ix=%s, but expected %s",
        ...
        if (img_info->ix != in_file_ix) {
            rt_err(strprintf("newly-loaded image had ix=%s, but expected %s",
                             str(img_info->ix).c_str(), str(in_file_ix).c_str()));
        }
    ...
    void load_pil_t::read_pascal_image_list_file(filename_t const &class_pil_fn, bool const load_imgs,
...
#pragma omp parallel for
    ...
    void load_pil_t::flickr_logos_load(void) {
        ...
        for (vect_string::iterator i = fl_list_lines->begin(); i != fl_list_lines->end(); ++i) {
            ...
            if (parts.size() != 2) {
                rt_err("failed to parse line in FlickrLogos-style image list. expected exactly 1 ',', had:" + *i);
            }
            ...
            for (vect_string::iterator i = bboxes_lines->begin(); i != bboxes_lines->end(); ++i) {
                ...
                gt_det.p[0].read_from_line_parts(box_parts, 0);
                gt_det.p[0] -= i32_pt_t{1, 1}; // 1-based, so adjust
                gt_det.p[1].read_from_line_parts(box_parts, 2);
                gt_det.p[1] += gt_det.p[0]; // read as size, so add nc to make pc
                assert_st(gt_det.is_strictly_normalized());
                gt_dets.push_back(gt_det);
                if (!gt_det.difficult) { ++gt_dets.num_non_difficult.v; }
            }
    ...
    string darknet_find_labels_fn_for_image_fn(string const &img_fn) {
        ...
        for (vect_string::const_iterator i = image_exts.begin(); i != image_exts.end(); ++i) {
            ...
            if (had_ext) {
                labels_fn = maybe_labels_fn + ".txt";
                break;
            }
        }
        if (labels_fn.empty()) {
            rt_err("image filename did not end with .png or .jpg (need to add new known image ext?): " + img_fn);
        }
    ...
    void load_pil_t::darknet_load(void) {
        ...
        for (vect_string::iterator i = fl_list_lines->begin(); i != fl_list_lines->end(); ++i) {
            ...
            p_istream labels_in = ifs_open(
                    labels_fn); // for now, we assume/assert that a GT label file will exist for every image
            string label;
    ...
    void load_pil_t::load_img_db(bool const load_imgs) {
        if (load_mode == "pascal") { // pascal load; handle first and return, since it loads imgs itself in subfunction
            ...
            for (vect_string::const_iterator i = classes->begin(); i != classes->end(); ++i) {
                ...
                read_pascal_image_list_file(filename_t_printf(pil_fn, (*i).c_str()), load_imgs && is_first_class,
                                            !is_first_class);
            }
...
#pragma omp parallel for
    ...
    match_res_t
    img_db_t::try_match(string const &class_name, p_per_class_scored_dets_t scored_dets, scored_det_t const &sd) {
        ...
        for (vect_gt_det_t::iterator i = gt_dets.begin(); i != gt_dets.end(); ++i) {
          ...
          printf( "sd=%s gt=%s sd_area=%s gt_area=%s gt_overlap=%s gt_score=%s max_score=%s hmo=%s\n", str(sd).c_str(), str(*i).c_str(),str(sd_area).c_str(), str(gt_area).c_str(), str(gt_overlap).c_str(), str(double(gt_overlap)/gt_sd_union).c_str(), str(double(best_score.d[0])/best_score.d[1]).c_str(), str(has_min_overlap).c_str() );
            ...
            if ((uint64_t(gt_overlap) * best_score.d[1]) > (uint64_t(best_score.d[0]) * gt_sd_union)) {
                best_score.d[0] = gt_overlap;
                best_score.d[1] = gt_sd_union;
                best_difficult = i->difficult;
                best = &gtms.at(i - gt_dets.begin());
            }
        }
        if (!best_score.d[0]) {
            return match_res_t(0, 0);
        }// no good overlap with any gt_det. note: difficult is unknown here
        if (best->matched) {
            assert_st(best->match_score >= sd.score);
            return match_res_t(0, best_difficult);
        }
        best->matched = 1;
    ...
    void
    print_prc_line(p_ostream const &out, prc_elem_t const &prc_elem, uint32_t const tot_num_class, double const &map) {
        ...
        for (vect_scored_det_t::const_iterator i = all_sds->begin(); i != all_sds->end();) {
            ...
            while (1) // handle all dets with equal score as a unit
            {
                ...
                if (match_res.is_pos && !match_res.is_diff) {
                    ++num_test;
                    ++num_pos; // positive, recall increased (precision must == 1 or also be increased)
                } else if (!match_res.is_pos) { ++num_test; } // negative, precision decreased
                ++i;
                if ((i == all_sds->end()) || (cur_score != i->score)) { break; }
            }
    ...
    void img_db_t::score_results(p_vect_p_per_class_scored_dets_t name_scored_dets_map,
        ...
        for (vect_p_per_class_scored_dets_t::iterator i = name_scored_dets_map->begin();
             i != name_scored_dets_map->end(); ++i) {
            double const class_ap = score_results_for_class(*i, prc_fn, plot_base_fn);
            if (summary_out) {
                (*summary_out)
                        << strprintf("class_name=%s ap=%s\n", str((*i)->class_name).c_str(), str(class_ap).c_str());
            }
            mean_ap += class_ap;
    ...
    struct is_comma {
        bool operator()(char const &c) const { return c == ','; }
    };
              ...
              public load_pil_t // NESI(help="hamming first-level cascade boxes analysis",bases=["load_pil_t"], type_id="ham_ana")
    {
        ...
        virtual void main(nesi_init_arg_t *nia) {
            ...
            (*summ_out) << strprintf(
                    "class_name,num_tot,ham_only,dpm_only,num_ham,num_dpm,num_both,num_either,num_neither,\n");
            for (uint32_t cix = 0; cix != classes->size(); ++cix) {
                ...
                for (uint32_t ix = 0; ix < img_db->img_infos.size(); ++ix) {
                    ...
                    vect_gt_match_t &ham_gtms = ham_sds->get_gtms(ix,
                                                                  gt_dets.size()); // must already exist, but not checked
                    vect_gt_match_t &dpm_gtms = dpm_sds->get_gtms(ix,
                                                                  gt_dets.size()); // must already exist, but not checked
                    bool unmatched = 0;
                ...
                (*summ_out) << strprintf("%s,%s,%s,%s,%s,%s,%s,%s,%s,\n", str(class_name).c_str(), str(num_tot).c_str(),
                                         str(ham_only).c_str(), str(dpm_only).c_str(), str(num_ham).c_str(),
                                         str(num_dpm).c_str(), str(num_both).c_str(), str(num_either).c_str(),
                                         str(num_neither).c_str());
            }
#endif
        }
    };
