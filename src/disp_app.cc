...
namespace boda {
        ...
        // bases=["load_pil_t"], type_id="display_pil")
    {
            ...
            while (1) {
                ...
                for (vect_string::const_iterator i = (*classes).begin(); i != (*classes).end(); ++i) {
                    ...
                    if (do_score) {
                        ...
                        if (img_sds) {
                            for (vect_base_scored_det_t::const_iterator i = img_sds->begin();
                                 i != img_sds->end(); ++i) {
                                //printf( "cn=%s i->score=%s\n", str(cn).c_str(), str(i->score).c_str() );
                                if (i->score > det_show_thresh) {
                                    annos->push_back(anno_t{*i, rgba_to_pel(40, 40, 170), 0, cn + "=" + str(i->score),
                                                            rgba_to_pel(220, 220, 255)});
                                }
                            }
                        }
                ...
                if (!keep_img) {
                    ...
                    if (cur_img_ix == start_img) {
                        printf("no images to show.\n");
                        break;
                    }
                } else {
                    disp_win.update_img_annos(0, annos);
        ...
        void on_lb(error_code const &ec) {
            ...
            if (0) {}
            else if (lbe.is_key && (lbe.keycode == 'd')) {
                mod_adj(cur_img_ix, img_db->img_infos.size(), 1);
                auto_adv = 0;
            }
                ...
                auto_adv = 0;
                p_img_info_t img_info = img_db->img_infos[cur_img_ix];
                printf("cur_img_ix=%s img_info->full_fn=%s img_info->id=%s\n", str(cur_img_ix).c_str(),
                       str(img_info->full_fn).c_str(), str(img_info->id).c_str());
                ...
                unknown_command = 1;
                printf("unknown/unhandled UI event\n");
            } // unknown command
        ...
        virtual void main(nesi_init_arg_t *nia) {
            ...
            if (do_score) {
                ...
                if (do_score == 1) { // use one set of rp_boxes as dets for all classes
                    ...
                    for (vect_string::const_iterator i = (*classes).begin(); i != (*classes).end(); ++i) {
                        scored_dets->push_back(
                                read_results_file(img_db, strprintf(res_fn.exp.c_str(), (*i).c_str()), *i));
                    }
                } else {
                ...
                img_db->score_results(scored_dets, prc_txt_fn.exp, prc_png_fn.exp, "",
                                      quit_after_score); // NOTE: no summary output
