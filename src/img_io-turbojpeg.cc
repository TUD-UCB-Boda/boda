...
namespace boda {
    ...
    void img_t::from_jpeg(p_uint8_with_sz_t const &data, std::string const &fn) {
        ...
        check_tj_ret_fn(!tj_dec, fn,
                        "tjInitDecompress"); // note: !tj_dec passed as tj_ret, since 0 is the fail val for tj_dec
        ...
        tj_ret = tjDecompress2(tj_dec, data.get(), data.sz, pels.get(), sz.d[0], row_pitch, sz.d[1], tj_pixel_format,
                               0);
    ...
    p_uint8_with_sz_t img_t::to_jpeg(void) {
        ...
        if (!yuv_pels.empty()) { // if yuv data present, use it instead of pels
            ...
            tj_ret = tjCompress2(tj_enc, pels.get(), sz.d[0], row_pitch, sz.d[1], tj_pixel_format, &tj_buf_out,
                                 &tj_size_out, TJSAMP_444, quality, 0);
