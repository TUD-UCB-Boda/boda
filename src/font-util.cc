...
namespace boda {
    ...
    struct ttf_font_render_t : public virtual nesi, public font_render_t // NESI(help="stb_truetype-based ttf-font-rendering to bitmaps", bases=["font_render_t"], type_id="ttf" )
    {
        ...
        void lazy_init(void) {
            ...
            int const font_offset = stbtt_GetFontOffsetForIndex(rp_font_data,
                                                                0); // FIXME: doesn't take len of data, so presumably unsafe ...
        ...
        virtual p_rendered_char_t render_char(int8_t const &c) {
            ...
            if (!rc) {
                ...
                rc->bitmap = p_uint8_t(
                        stbtt_GetCodepointBitmap(&font, scale, scale, c, &rc->sz.d[0], &rc->sz.d[1], &rc->xoff,
                                                 &rc->yoff), free);
                int advance, lsb;
