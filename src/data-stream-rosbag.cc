...
namespace boda {
        ...
        // bases=["data_stream_t"], type_id="rosbag-src")
    {
        ...
        void msg_to_db(message_instance_t const *const prim_msg, bool const &is_stale,
            ...
            if (!prim_msg) {
                assert_st(!is_stale);
            } // should emit every primary msg, so it should never be marked stale
            if (msg.getDataType() == "sensor_msgs/Image") {
                ...
                if (img->encoding != "bayer_bggr8") {
                    rt_err("unsupported image encoding in rosbag: " + img->encoding);
                }

                assert_st((img->height * img->step) == img->data.size());
                p_nda_uint8_t img_nda = make_shared<nda_uint8_t>(
                        dims_t{vect_uint32_t{uint32_t(img->height), uint32_t(img->width)}, vect_string{"y", "x"},
                               "uint8_t"}); // note: for now, always in in bggr format ...
                ...
                for (auto i = pc2->fields.begin(); i != pc2->fields.end(); ++i) {
                    for (auto gf = pc2_gfs.begin(); gf != pc2_gfs.end(); ++gf) {
                        if (i->name == gf->name) {
                            ...
                            if (i->count != 1) {
                                rt_err("unsupported not-equal-to-one PointField count: " + str(i->count));
                            }
                            gf->offset = i->offset;
                            ++gfs_found;
                        }
                ...
                if (gfs_found != pc2_gfs.size()) {
                    rt_err(strprintf(
                            "can't parse PointCloud2, only found gfs_found=%s fields, but needed pc2_gfs.size()=%s fields.",
                            str(gfs_found).c_str(), str(pc2_gfs.size()).c_str()));
                }
                geometry_msgs::TransformStamped tf = tf2_bc->lookupTransform(frame_id, pc2->header.frame_id, ros::Time(
                        0)); // note: use last-known transform, not 'correct' one, to avoid 'extrapolate into past' error. was: // pc2->header.stamp );
                tf2::doTransform(*pc2, pc2_tf_buf, tf);
                p_nda_float_t pc2_nda = make_shared<nda_float_t>(
                        dims_t{vect_uint32_t{uint32_t(pc2_tf_buf.height), uint32_t(pc2_tf_buf.width),
                                             uint32_t(pc2_gfs.size())}, vect_string{"y", "x", "p"}, "float"});
                ...
                if (marker.type == 1) {
                    ...
                    geometry_msgs::TransformStamped tf = tf2_bc->lookupTransform(frame_id, marker.header.frame_id,
                                                                                 ros::Time(
                                                                                         0)); // note: use last-known transform, not 'correct' one, to avoid 'extrapolate into past' error. was: // marker->header.stamp );
                ...
                for (uint32_t i = 0; i != marker_buf.size(); ++i) {
                    ...
                    bool const keep = ts_delta(cur_time, marker_ts) <
                                      get_ros_timestamp(m.lifetime); // FIXME: real duration version
                    if (keep) {
                        *o = m;
                        o++;
                    }
                ...
                if (marker_buf.size()) {
                    ...
                    p_nda_float_t marker_nda = make_shared<nda_float_t>(
                            dims_t{vect_uint32_t{uint32_t(marker_buf.size()), 3}, vect_string{"v", "p"}, "float"});
                    for (uint32_t i = 0; i != marker_buf.size(); ++i) {
                        auto const &m = marker_buf[i];
                ...
                ret.nda = make_shared<nda_t>(
                        dims_t{vect_uint32_t{uint32_t(rsd.buf.size())}, vect_string{"v"}, "uint8_t"});
        ...
        // bases=["data_stream_t"], type_id="rosbag-sink")
    {
        ...
        void write_db_to_bag(data_block_t const &db, string const &topic) {
            ...
            ros::Time ros_ts;
            ros_ts.fromNSec(
                    db.timestamp_ns); // note: we'll use this for both the 'recv' and 'header' timestamp for our gen'd message
