...
namespace boda {
    ...
    struct lexp_stats_t {
        ...
        void set_to_zeros(void) {
            ...
            num_list = 0;
            num_kids = 0;
        } // no ctor so we can use {} init; --std=c++-sigh
        bool is_eq(lexp_stats_t const &o) {
            return num_leaf == o.num_leaf && num_list == o.num_list && num_kids == o.num_kids;
        }
    };

    std::ostream &operator<<(std::ostream &os, lexp_stats_t const &v) {
        return os << strprintf("num_leaf=%s num_list=%s num_kids=%s", str(v.num_leaf).c_str(), str(v.num_list).c_str(),
                               str(v.num_kids).c_str());
    }

    void lexp_t::get_stats(lexp_stats_t &lex_stats) {
        if (leaf_val.exists()) {
            ++lex_stats.num_leaf;
            assert_st(kids.empty());
        }
        else {
            ++lex_stats.num_list;
    ...
    void lexp_name_val_map_t::insert_leafs_from(vect_pair_str_str const &kvs) {
        for (vect_pair_str_str::const_iterator i = kvs.begin(); i != kvs.end(); ++i) {
    ...
    void lexp_name_val_map_t::init_nvm(void) {
        if (!l) {
            nvm_init = 1;
            return;
        } // trivial/null init for no-lexp use-case
        assert_st(l);
    ...
    p_lexp_t lexp_name_val_map_t::find(char const *n, lexp_name_val_map_t **const found_scope) {
        ...
        if (nvm_init) { // skip uninited nvms (from vectors, leafs, etc)
            ...
            if (nvmi != nvm.end()) {
                *found_scope = this;
                return nvmi->second;
            } // if found at this scope, return value
        }
        ...
        while (cur_c() != ')') {
            ...
            if (startswith(kid.n.str(), xml_inc_prefix)) {
                if (!kid.v->leaf_val.exists()) { // FIXME: better error?
                    rt_err("invalid attempt to use name/value list as filename for XML include. list was:" +
                           str(*kid.v));
                }
    ...
    void lex_parse_t::parse_leaf(p_lexp_t ret) {
        ...
        if (had_escape) { ret->leaf_val.set_from_string(cooked); }
            ...
            ret->leaf_val = ret->src;
            ret->leaf_val.shrink_end_off(cur_off());
        }
    }
        ...
        for (vect_string::const_iterator ai = b; ai != e; ++ai) {
            ...
            if (!startswith(arg, "--")) {
                ...
                for (string::const_iterator i = arg.begin() + 2; i != arg.end(); ++i) {
                    if ((*i) == '=') {
                        if ((i + 1) == arg.end()) {
                            os << strprintf("warning empty/missing value after '=' in option '%s'\n",
                                            arg.c_str());
                        }
                ...
                if (!key_had_eq) {
                    if ((ai + 1) == e) {
                        rt_err(strprintf("missing value for option '%s': no '=' present, and no more args",
                                         arg.c_str()));
                    }
                    val = string(*(ai + 1));
                    ++ai;
    ...
    size_t str_format_find_next_ref(string &ref, string *const out, string const &fmt, size_t const &spos) {
        ...
        for (size_t i = spos; i < fmt.size(); ++i) {
            if (fmt[i] == '%') {
                ++i;
                if (!(i < fmt.size())) { rt_err("end of input after '%', expected '(' or '%'."); }
                if (fmt[i] == '%') { if (out) { out->push_back(fmt[i]); }} // escaped '%'
                    ...
                    while (1) {
                        ...
                        if (fmt[i] != ')') { ref.push_back(fmt[i]); }
                            ...
                            ++i;
                            return i;
                        } // done, return next spos (i.e. index after last consumed pos)
                    }
                }
        ...
        while (1) {
            ...
            if (spos != string::npos) { // found a ref, process it; expand ref recursively.
                ...
                if (!di->leaf_val.exists()) {
                    rt_err("invalid attempt to use name/value list as filename ref '" + ref + "' value. list was:" +
                           str(*di));
                }
                ++di->use_cnt;
            ...
            try {
                ...
                if (!lt.err_fmt) { test_fail_err(rte.err_msg); } // expected no error, but got one
                    ...
                    if (rte.err_msg != exp_err_msg) {
                        test_fail_wrong_err(
                                strprintf("  %s\nexpected:\n  %s\n", str(rte.err_msg).c_str(),
                                          str(exp_err_msg).c_str()));
                    }
                }
            }
            if (test_ret) {
                if (lt.err_fmt) { test_fail_no_err(*lt.err_fmt); }
                    ...
                    if (lexp_to_str != lt.in) {
                        test_fail_wrong_res(strprintf("lexp_to_str=%s != lt.in=%s\n",
