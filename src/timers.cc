...
namespace boda {
    ...
    uint64_t get_cur_time(void) {
        ...
        while (clock_gettime(cur_clock, &ts)) {
            if (errno == EINVAL) {
                if (cur_clock != CLOCK_MONOTONIC) {
                    rt_err("neither CLOCK_MONOTONIC_RAW nor CLOCK_MONOTONIC supported");
                }
        ...
        assert_st(ts.tv_sec <
                  1000 * 1000 * 1000); // sanity check: process run time < 1B seconds. keeps total nsec in 64 bits
    ...
    struct tlog_map_val_by_bt_comp {
        bool operator()(tlog_map_val_t const &a, tlog_map_val_t const &b) const {
            return a.second.min_bt < b.second.min_bt;
        }
    };
