...
namespace boda {
    ...
    void img_t::load_fn(std::string const &fn) {
        ...
        if (0) {}
            ...
            rt_err("failed to load image '" + fn + "': could not auto-detect file-type from extention."
    ...
    void img_t::set_sz(u32_pt_t const &sz_) {
        ...
        if (!sz.both_dims_non_zero()) {
            rt_err(strprintf("can't create zero-area image. requests WxH was %sx%s", str(sz.d[0]).c_str(),
                             str(sz.d[1]).c_str()));
        }
    ...
    void img_t::set_sz_and_alloc_pels(u32_pt_t const &sz_) {
        set_sz(sz_);
        alloc_pels();
    }
    ...
    void img_t::set_sz_and_pels_from_yuv_420_planes(vect_p_nda_uint8_t const &yuv_ndas) {
        ...
        for (uint32_t pix = 0; pix != 3; ++pix) {
            ...
            if (!pix) {
                ...
                if (yuv_sz.scale(2) != sz) {
                    rt_err(strprintf("yuv plane size mismatch. Y sz=%s, but for pix=%s, yuv_sz=%s\n",
...
#pragma omp parallel for
    ...
    void img_t::load_fn_jpeg(std::string const &fn) {
        ...
        p_uint8_with_sz_t mfile_data(mfile, (uint8_t *) mfile->data(),
                                     mfile->size()); // alias ctor to bind lifetime to mapped file
    ...
    void img_t::load_fn_png(std::string const &fn) {
        ...
        if (ret) {
            rt_err(strprintf("failed to load image '%s': lodepng decoder error %s: %s",
        ...
        for (uint32_t i = 0; i < sz.d[1]; ++i) {
            memcpy(pels.get() + i * row_pitch, (&lp_pels[0]) + i * pack_rl, pack_rl);
        }
    }
    ...
    void img_t::save_fn_png(std::string const &fn, bool const disable_compression) {
        ...
        uint32_t const pack_rl = sz.d[0] * lp_depth;
        for (uint32_t i = 0; i < sz.d[1]; ++i) {
            memcpy((&lp_pels[0]) + i * pack_rl, pels.get() + i * row_pitch, pack_rl);
        }
        ...
        if (ret) {
            rt_err(strprintf("failed to encode image '%s': lodepng decoder error %s: %s",
                             fn.c_str(), str(ret).c_str(), lodepng_error_text(ret)));
        }
                ...
                for (uint32_t c = 0; c < 3; ++c) {
                    dest_val +=
                            uint32_t(uint8_t(((
                                                      ...
                                                      ((get_chan_64(c, src_data_m) << 30)) +
                                                      // note: src_data_m may be zero
...
#pragma omp parallel for
    ...
    void img_draw_pels(img_t *const dest, u32_pt_t const &d, uint32_t const &num,
        ...
        for (uint32_t i = 0; i < num; ++i) {
            *dest_data = interp_pel(ic, ec, num, i);
            dest_data += stride_pels;
        }
    }
