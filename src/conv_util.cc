...
namespace boda {
    ...
    conv_op_info_t const Spreading_coi{"Spreading", {"out", "out_grad_loss", "in"}, {"in_grad_loss"},
                                       DefaultPoolingVals};
    conv_op_info_t const ZeroIfNonPos_coi{"ZeroIfNonPos", {"in", "cond"},
                                          {"out"}}; // note: dims(cond)==dims(out)==dims(in);out=(cond>=0)?in:0
    ...
    vect_rp_conv_op_info_t conv_op_infos{&clone_coi, &sgemm_coi,
                                         ...
                                         &Dropout_coi, &LRN_coi,
                                         &Accuracy_coi, &Softmax_coi, &SoftmaxWithLoss_coi, &Data_coi, &Concat_coi,
                                         &Reduce_coi, &Eltwise_coi,
                                         &Split_coi,
    ...
    string conv_op_info_t::bot_an(uint32_t const &ix) const {
        if (has_var_bots.v) {
            ...
            return bots[0] + "_" + str(ix);
        }
        else {
            assert_st(ix < bots.size());
            return bots[ix];
        }
    }
    ...
    string conv_op_info_t::top_an(uint32_t const &ix) const {
        if (has_var_tops.v) {
            ...
            return tops[0] + "_" + str(ix);
        }
        else {
            assert_st(ix < tops.size());
            return tops[ix];
        }
    }

    // type string checking + verify input/output argument count and other sanity checks
    bool conv_op_base_t::is(conv_op_info_t const &coi_) const {
        assert_st(coi);
        return coi == &coi_;
    }
    ...
    u32_pt_t conv_op_t::in_sz_to_out_sz(u32_pt_t const &in_sz, bool const ignore_padding) const {
        if (!has("kern_sz")) { // handle non-conv cases
            ...
            if (is(Pooling_coi) || is(InnerProduct_coi)) {
                return u32_pt_t{1, 1};
            } // global pooling / inner product special cases
    ...
    u32_pt_t conv_op_t::out_sz_to_in_sz(u32_pt_t const &out_sz, bool const ignore_padding) const {
        if (!has("kern_sz")) { // handle non-conv cases
            ...
            if (is(Pooling_coi) || is(InnerProduct_coi)) { // inner product and global pooling special cases
                if (out_sz != u32_pt_t{1, 1}) {
                    rt_err("global pooling layer can't produce an out_sz other than {1,1}");
                }
    ...
    dims_t conv_pipe_t::get_data_img_dims(void) const {
        if (data_img_node_names.size() != 1) {
            rt_err("not exactly one data img input node in net; can't process. data img input nodes are: " +
                   str(data_img_node_names));
        }
    ...
    u32_pt_t conv_pipe_t::get_data_img_xy_dims_3_chans_only(void) const {
        ...
        if (data_dims_chan != 3) {
            rt_err("unsupported number of fata img input node chans; must == 3; saw '" + str(data_dims_chan) + "'");
        }
    ...
    p_conv_node_t conv_pipe_t::get_single_top_node(void) const {
        if (out_node_name.empty()) {
            if (tops.size() != 1) {
                rt_err("not exactly one sink/output node in net; can't process. output nodes are: " + str(tops));
            }
            return must_get_node(*tops.begin());
        } else {
            if (!has(*nodes, out_node_name)) {
                rt_err("node '" + out_node_name +
                       "' specified for use as producing the primary net output not found in net.");
            }
    ...
    p_conv_node_t conv_pipe_t::get_or_make_node(string const &name, bool const is_bot, bool const is_top) {
        ...
        if (!ret) {
            ret.reset(new conv_node_t{name});
            tops.insert(name);
            bots.insert(name);
        }
        if (is_bot) { tops.erase(name); }
        if (is_top) { bots.erase(name); }
        return ret;
    }
    ...
    void conv_pipe_t::calc_support_forward_op(p_conv_op_t const &cop, bool const ignore_padding) {
        ...
        if (cop->is(BckConv_coi)) {
            ...
            for (vect_string::const_iterator j = cop->bots.begin(); j != cop->bots.end(); ++j) {
                ...
                if (!csi_in.valid()) {
                    rt_err("calc_support_info(): needed input support info for node not set. node name: " + str(*j));
                }
                if ((j == cop->bots.begin()) || (csi_in.support_stride.dims_max() >
                                                 csi_out.support_stride.dims_max())) { // first input or bigger stride
                    if (j != cop->bots.begin()) {
                    ...
                    if (csi_in.support_stride == csi_out.support_stride) {
                        csi_out.support_sz.max_eq(csi_in.support_sz);
                    }
                }
            ...
            if (!cop->is(Convolution_coi)) {
                if (cop->bots.size() != 1) {
                    printstr("warning: calc_support_forward_op(): unhandled multi-input operation: " + cop->tag +
                             " of type " + cop->get_type() + ". Will propogate support info from first input only.\n");
                }
            }
            p_conv_node_t const &j_node = must_get_node(cop->bots[0]);
            conv_support_info_t const &csi_in = j_node->csi;
            if (!csi_in.valid()) {
                rt_err("calc_support_info(): needed input support info for node not set. node name: " +
                       str(cop->bots[0]));
            }
            u32_pt_t const in_sz_1x1 = cop->out_sz_to_in_sz(u32_pt_t(1, 1),
                                                            ignore_padding); // == cop.kern_sz (if ign_pad)
    ...
    void conv_pipe_t::calc_support_forward_rec(string const &node_name, bool const ignore_padding) {
        ...
        for (vect_string::const_iterator i = node->bot_for.begin(); i != node->bot_for.end(); ++i) {
            ...
            for (vect_string::const_iterator i = cop->tops.begin();
                 i != cop->tops.end(); ++i) { calc_support_forward_rec(*i, ignore_padding); }
        }
    }
    ...
    void conv_pipe_t::calc_support_info(bool const ignore_padding) {
        ...
        for (set_string::const_iterator i = bots.begin(); i != bots.end(); ++i) {
            calc_support_forward_rec(*i, ignore_padding);
        }
    }
    ...
    void conv_pipe_t::calc_dims_op(p_conv_op_t const &cop) {
        ...
        if (dims_out.size()) {
            rt_err("calc_dims_op(): unhandled: out dims already set (node with multiple writers):" + node_out->name);
        }

        if (cop->is(
                BckConv_coi)) { // { in, filts, biases, out_grad_loss } --> { in_grad_loss, filts_grad_loss, biases_grad_loss }
            for (uint32_t i = 0; i != 3; ++i) { // propogate # chans
                dims_t &od = must_get_node(cop->tops[i])->dims;
                if (od.size()) {
                    rt_err("calc_dims_op(): unhandled: out dims already set (node with multiple writers):" +
                           cop->tops[i]);
                }
            ...
            loss_dims = dims_t(vect_uint32_t{1, 1}, vect_string{"y", "x"},
                               dims_out.tn); // note: loss gets same type as SM output
            ...
            dims_t implied_label_dims(vect_uint32_t {
                dims_out.dsz("img"), dims_out.dsz("y"), dims_out.dsz("x")
            }, vect_string{"img", "y", "x"}, "float" );
            dims_t &label_dims = must_get_node(cop->bots[1])->dims;
            if (label_dims.empty()) { label_dims = implied_label_dims; }
            else if (label_dims != implied_label_dims) {
                rt_err("error: label used by multiple SoftmaxWithLoss layers with differing xy size or # imgs");
            }
            ...
            if (label_max_val != implied_label_max_val) {
            ...
            for (vect_string::const_iterator j = cop->bots.begin(); j != cop->bots.end(); ++j) {
                ...
                if (!dims_out.size()) {
                    dims_out = j_dims;
                    dims_out.clear_strides();
                    dims_out.must_get_dim_by_name("chan").sz = 0;
                } // convert to template
                    ...
                    rt_err("concat layer had incompatible inputs; must have all same non-chan dims. template (from first input) was: " +
                           str(dims_out) + ". mismatching input was (index=" + str(j - cop->bots.begin()) + "): " +
                           str(j_dims));
                }
            }
            ...
            if (cop->is(Convolution_coi) || cop->is(Deconvolution_coi)) {
                ...
                dims_t filts_dims(vect_uint32_t {
                    cop->get_u32("out_chans"), j_node->dims.dsz("chan"), kern_sz.d[1], kern_sz.d[0]
                },
                ...
                if (cop->bots.size() != 1) {
                    rt_err("calc_dims(): unhandled multi-input operation: " + cop->tag + " of type " + cop->get_type() +
                           " ");
                }
            ...
            dims_out.must_get_dim_by_name("chan").sz = out_chans ? out_chans : dims_in.dsz(
                    "chan"); // reset or propogate num_chans
    ...
    void conv_pipe_t::calc_sizes_back_rec(p_conv_node_t const &node_out, bool const ignore_padding) {
        ...
        if (xy_dims_in.is_zeros()) {
            rt_err("internal error: !cio_in.valid() in calc_sizes_back_rec() at node:" + node_out->name);
        }
    ...
    void print_op_decl(std::ostream &out, conv_pipe_t const *const pipe, p_conv_op_t const &cop) {
        ...
        out << strprintf("net.add_op( %s(name=\"%s\",bot_names=%s,top_names=%s%s) )\n",
                         cop->get_type().c_str(), tag_id, as_py_str_list(cop->bots).c_str(),
                         as_py_str_list(cop->tops).c_str(), str_vals.c_str());
    }
...
//	  && !is_reduce_in( this, node )  
    ...
    void conv_pipe_t::add_bck_ops_op(vect_p_conv_op_t &bck_ops, p_conv_op_t const &cop) {
        ...
        if (cop->is(Softmax_coi)) { assert_st(0); }
            ...
            cop->erase("emit_out_in_yx");
            cop->set_u32("emit_out_in_yx", 1);
            ...
            *bcop = *cop;
            ...
            bcop->bots.push_back(
                    bcop->tops[0]); // take original input as input (need size and which-elem-is-max per window) could use mask instead)
            bcop->tops[0] = get_grad_loss_onn(cop,
                                              bcop->tops[0]); // note: pooling has no params, so there is second output for parameter gradients (as with some bck ops)
            ...
            *bcop = *cop;
            ...
            bcop->tops[0] = get_grad_loss_onn(cop,
                                              bcop->tops[0]); // note: ReLU has no params, so there is second output for parameter gradients (as with some bck ops)
            ...
            *bcop = *cop;
            ...
            cop->erase("emit_out_scale_base");
            cop->set_u32("emit_out_scale_base", 1);
            bcop.reset(new conv_op_t);
            *bcop = *cop;
            bcop->coi = 0;
    ...
    void conv_pipe_t::add_bck_ops_rec(vect_p_conv_op_t &bck_ops, string const &node_name) {
        ...
        if (node->bot_for.size() == 0) {
            ...
            if (!get_op(node->top_for[0])->is(SoftmaxWithLoss_coi)) {
                rt_err(strprintf("add_bck_ops: unhandled: top node %s not produced by SoftmaxWithLoss op",
                                 node_name.c_str()));
            }
        }
        for (vect_p_conv_op_t::const_reverse_iterator j = node->in_place_ops.rbegin();
             j != node->in_place_ops.rend(); ++j) {
    ...
    void
    conv_pipe_t::run_setup_input(p_nda_float_t const &in, p_map_str_p_nda_float_t const &fwd, vect_string &in_vns) {
        if (data_img_node_names.size() != 1) {
            rt_err("run_one_blob_in_one_blob_out only supports exactly one image input");
        }
        ...
        if (data_label_node_names.size()) {
            ...
            uint32_t lix = 0;
            for (dims_iter_t di(label->dims);;) {
                label->at(di.di) = lix % label_cio.max_val;
                ++lix;
                if (!di.next()) { break; }
            }
            (*fwd)[lnn] = label;
    ...
    struct conv_ana_t
            : virtual public nesi,
              public has_main_t // NESI(help="analysize pipeline of convolutions wrt sizes at each layer, strides, padding, and per-layer-input-sizes (aka support sizes). ",bases=["has_main_t"], type_id="conv_ana")
    {
        ...
        virtual void main(nesi_init_arg_t *nia) {
            ...
            data_img_node->dims = dims_t(vect_uint32_t{1, in_chans, in_sz ? *in_sz : 1, in_sz ? *in_sz : 1},
    ...
    p_net_param_t conv_pipe_t::as_net_param(void) const {
        assert(orig_net_param);
        return orig_net_param;
    }
