...
namespace boda {
    void cl_err_chk(cl_int const &ret, char const *const tag) {
        if (ret != CL_SUCCESS) {
            rt_err(strprintf("%s() failed with ret=%s (%s)", tag, str(ret).c_str(), get_cl_err_str(ret)));
        }
    }


    template<typename T>
    void debug_rr(string const &tag, T const &v) {
    ...
    template<typename T>
    struct cl_release_T {
        typedef cl_int (*p_func)(T);
    };

    template<typename T, typename cl_release_T<T>::p_func RELEASE, typename cl_release_T<T>::p_func RETAIN>
    struct cl_wrap_t {
        T v;
        ...
        void reset(void) {
            if (valid()) {
                ...
                cl_int const ret = RELEASE(v);
                v = 0;
                cl_err_chk(ret, "clRelease<T>");
            }
        }

        void reset(T const &v_) {
            reset();
            v = v_;
        } // assume ref cnt == 1
        ~cl_wrap_t(void) {
    ...
    struct ProgramBuild_t {
        ...
        ProgramBuild_t(cl_program const &p_, cl_device_id const &d_, cl_program_build_info const &i_) : p(p_), d(d_),
                                                                                                        i(i_) {}
    ...
    struct KernelWorkGroup_t {
        ...
        KernelWorkGroup_t(cl_kernel const &k_, cl_device_id const &d_, cl_kernel_work_group_info const &i_) : k(k_),
                                                                                                              d(d_),
                                                                                                              i(i_) {}
    ...
    T get_info(GI const &gi) {
        T ret;
        cl_int err = gi(sizeof(ret), &ret, 0);
        cl_err_chk(err, "clGet...Info()");
        return ret;
    }

    template<typename T, typename GI>
    T get_info_vect(GI const &gi) {
        T ret;
        size_t sz;
        cl_int err;
        err = gi(0, 0, &sz);
        cl_err_chk(err, "clGet...Info(get_size_of_var_sized_thing)");
    ...
    string get_info_str(GI const &gi) { return get_info_vect<string, GI>(gi); }

    cl_ulong get_prof_info(cl_event_t const &event, cl_profiling_info const &pn) {
        cl_ulong ret;
        cl_int err = clGetEventProfilingInfo(event.v, pn, sizeof(ret), &ret, 0);
    ...
    void cl_get_platforms(vect_cl_platform_id &out) {
        ...
        cl_uint sz;
        cl_int err;
        err = clGetPlatformIDs(0, 0, &sz);
        cl_err_chk(err, "clGetPlatformIDs(size)");
        out.resize(sz);
        err = clGetPlatformIDs(sz, &out[0], 0);
        cl_err_chk(err, "clGetPlatformIDs(out)");
    }
    ...
    void cl_err_chk_build(cl_int const &ret, cl_program const &program, vect_cl_device_id const &use_devices) {
        if (ret != CL_SUCCESS) {
            for (vect_cl_device_id::const_iterator i = use_devices.begin(); i != use_devices.end(); ++i) {
                ...
                string const bs = str(
                        get_info<cl_build_status>(ProgramBuild_t(program, device, CL_PROGRAM_BUILD_STATUS)));
                ...
                printf("OpenCL build error (for device \"%s\"): build_status=%s build_options=\"%s\" build_log:\n%s\n",
                       get_info_str(Device_t(device, CL_DEVICE_NAME)).c_str(), str(bs).c_str(), str(bo).c_str(),
                       str(bl).c_str());
            }
    ...
    template<typename V>
    void set_kernel_arg(cl_kernel_t const &k, uint32_t const &ai, V const &v) {
        cl_int err;
        err = clSetKernelArg(k.v, ai, sizeof(v), &v);
        ...
        // bases=["rtc_compute_t"], type_id="ocl" )
    {
        ...
        void init(void) {
            ...
            for (vect_cl_platform_id::const_iterator i = platforms.begin(); i != platforms.end(); ++i) {
                ...
                if (!devices.empty()) {
                    use_devices = vect_cl_device_id{devices[0]};
                } // pick first device only (arbitrarily)
            }
            ...
            cq.reset(clCreateCommandQueue(context.v, use_devices[0], CL_QUEUE_PROFILING_ENABLE,
                                          &err)); // note: not out of order
        ...
        virtual string get_plat_tag(void) {
            ...
            return "ocl:" + string(get_info_str(
                    Device_t(use_devices[0], CL_DEVICE_NAME)).c_str()); // FIXME: check-for and strip NULL earlier?
        }
        ...
        void compile(vect_rtc_func_info_t const &func_infos, rtc_compile_opts_t const &opts) {
            ...
            if (gen_src) {
                ...
                p_ostream out = ofs_open(
                        strprintf("%s/out_%s.cl", gen_src_output_dir.exp.c_str(), str(compile_call_ix.v).c_str()));
            ...
            err = clBuildProgram(prog.v, use_devices.size(), &use_devices[0],
                                 "-cl-fast-relaxed-math -cl-denorms-are-zero", 0, 0);
            ...
            if (gen_src) {
                ...
                cl_int const sz_err = Program_t(prog.v, CL_PROGRAM_BINARY_SIZES)(pb_szs.size() * sizeof(pb_szs[0]),
                                                                                 &pb_szs[0], 0);
                ...
                write_whole_fn(
                        strprintf("%s/out_%s.clb", gen_src_output_dir.exp.c_str(), str(compile_call_ix.v).c_str()),
                        ocl_bin);
            }
            for (vect_rtc_func_info_t::const_iterator i = func_infos.begin(); i != func_infos.end(); ++i) {
        ...
        void copy_nda_to_var(string const &vn, p_nda_t const &nda) {
            ...
            cl_int const err = clEnqueueWriteBuffer(cq.v, vi.buf.v, 1, 0, buf_sz, nda->rp_elems(), 0, 0,
                                                    0);  // note: blocking write
        ...
        void copy_var_to_nda(p_nda_t const &nda, string const &vn) {
            ...
            size_t const buf_sz = get_info<size_t>(MemObject_t(vi.buf.v, CL_MEM_SIZE));
            assert_st(buf_sz == nda->dims.bytes_sz());
            cl_int const err = clEnqueueReadBuffer(cq.v, vi.buf.v, 1, 0, buf_sz, nda->rp_elems(), 0, 0,
                                                   0); // note: blocking_read=1
        ...
        cl_event_t &get_call_ev(uint32_t const &call_id) {
            assert_st(call_id < call_evs.size());
            return call_evs[call_id];
        }

        uint32_t alloc_call_id(void) {
            call_evs.push_back(cl_event_t());
            return call_evs.size() - 1;
        }

        virtual void
        release_per_call_id_data(void) { call_evs.clear(); } // invalidates all call_ids inside rtc_func_call_t's
        ...
        void add_arg(rtc_arg_t const &arg, cl_kernel_t const &kern, uint32_t &cur_arg_ix) {
            ...
            if (arg.is_var()) { // pass-by-reference case
                ...
                } // null case (REFs, optional vars)
                else { // pass-by-value case
                    cl_int err;
                    err = clSetKernelArg(kern.v, cur_arg_ix, arg.v->dims.bytes_sz(), arg.v->rp_elems());
        ...
        uint32_t run(rtc_func_call_t const &rfc) {
            ...
            for (vect_string::const_iterator i = ofi.info.arg_names.begin(); i != ofi.info.arg_names.end(); ++i) {
                ...
                if (ai == rfc.arg_map.end()) {
                    rt_err(strprintf("ocl_compute_t: arg '%s' not found in arg_map for call.\n",
                                     str((*i)).c_str()));
                }
            ...
            size_t const kwgs = get_info<size_t>(
                    KernelWorkGroup_t(ofi.kern.v, use_devices[0], CL_KERNEL_WORK_GROUP_SIZE));
            // printf( "kwgs=%s\n", str(kwgs).c_str() ); // might be handy to see; might indicate occupancy limits for kernel
            if (loc_work_sz > kwgs) {
                unsup_err(strprintf(
                        "Can't run kernel: loc_work_sz is %s but OpenCL says max is %s for this kernel+device.",
