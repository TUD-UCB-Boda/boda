...
namespace boda {
        ...
        // bases=["data_stream_tagged_frames_t"], type_id="ffmpeg-src")
    {
        ...
        virtual void data_stream_init(nesi_init_arg_t *const nia) {
            ...
            if (err < 0) {
                rt_err(strprintf("avformat_open_input failed for ffmpeg_url=%s\n", str(ffmpeg_url).c_str()));
            }
            ...
            if (err < 0) {
                printf("warning: avformat_find_stream_info() failed for ffmpeg_url=%s\n", str(ffmpeg_url).c_str());
            }

            if (!(stream_index < ic->nb_streams)) {
                rt_err(strprintf(
                        "user requested (zero-based) stream_index=%s, but ffmpeg says there are only ic->nb_streams=%s streams.\n",
            ...
            if (avmss_ret == 0) {
                rt_err(strprintf("stream stream_index=%s is not a video stream", str(stream_index).c_str()));
            }
        ...
        void init_video_stream_decode(AVStream *const vid_st) {
            ...
            if (!codec) {
                rt_err(strprintf("no codec could be found for id avctx->codex_id=%s\n", str(avctx->codec_id).c_str()));
            }
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            ...
            while (1) {
                ...
                if (err < 0) { return ret; }
                assert_st((uint32_t) pkt.stream_index ==
                          stream_index); // AVDISCARD_ALL setting for other streams in init() should guarentee this
                if (out_dims) { // raw mode
                    ret.nda = make_shared<nda_t>(
                            dims_t{vect_uint32_t{uint32_t(pkt.size)}, vect_string{"v"}, "uint8_t"});
                ...
                if (got_frame) {
                    ...
                    for (uint32_t pix = 0; pix != 3; ++pix) {
                        ...
                        p_nda_uint8_t yuv_nda = make_shared<nda_uint8_t>(
                                dims_t{vect_uint32_t{uint32_t(ph), uint32_t(pw)}, vect_string{"y", "x"}, "uint8_t"});
        ...
        // bases=["data_stream_t"], type_id="ffmpeg-sink")
    {
        ...
        virtual string get_pos_info_str(void) {
            return strprintf("data_stream_ffmpeg_sink: tot_num_read=%s",
        ...
        void lazy_init(data_block_t const &db) {
            ...
            if (!codec) {
                rt_err(strprintf("avcodec_find_encoder_by_name() for codec=%s failed", str(codec_name).c_str()));
            }

            // set up format and format context
            AVOutputFormat *const out_fmt = av_guess_format(out_fmt_name.c_str(), 0, 0);
            if (!out_fmt) {
                rt_err(strprintf("av_guess_format() for out_fmt_name=%s failed", str(out_fmt_name).c_str()));
            }
            err = avformat_alloc_output_context2(&ofc, out_fmt, 0, 0);
            if (err || (!ofc)) { rt_err("avformat_alloc_output_context2() failed"); }

            ostr = avformat_new_stream(ofc,
                                       codec); // note: use codec here. this is will setup the context inside the stream for this codec
            if (!ostr) { rt_err("av_new_stream() failed"); }
            assert_st(ostr->id == (int) ofc->nb_streams -
                                  1); // FIXME/NOTE: is this really not set properly by avformat_new_stream?if not, set.
            ...
            if (ofc->oformat->flags &
                AVFMT_GLOBALHEADER) { octx->flags |= AV_CODEC_FLAG_GLOBAL_HEADER; } // BLACK MAGIC!
...
//      av_dict_set( &o, "preset", "ultrafast", 0 );
            ...
            if (err) {
                rt_err(strprintf("avio_open(): failed for file '%s' (expanded: '%s')", fn.in.c_str(), fn.exp.c_str()));
            }
            printf("fn.exp=%s\n", str(fn.exp).c_str());
        ...
        virtual data_block_t proc_block(data_block_t const &db) {
            ...
            if (!db.valid() && !img) {
                data_stream_deinit();
                return db;
            } // treat null db as deinit/end-of-video. any more packets
            if (!img) { rt_err("ffmpeg-sink: expected a data block with an image."); } // FIXME: can't happen now?
            if (img->yuv_pels.size() != 3) {
                rt_err("ffmpeg-sink: expected data block image to have 3 yuv_pels vectors.");
            }
            if (!ofc) { lazy_init(db); }
