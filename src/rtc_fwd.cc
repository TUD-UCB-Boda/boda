...
namespace boda {
    ...
    vect_string conv_pipe_fwd_t::gen_op_stats(string const &top_in) {
        ...
        for (uint32_t i = 0; i != reds.size(); ++i) {
            cur_ins.push_back(top_in_reshape);
        } // initial inputs are all top_in
        while (in_sz > 1) {
    ...
    void conv_pipe_fwd_t::gen_op_quantize(string const &top_in, uint32_t const &max_val, uint32_t const &keep_bits) {
        ...
        p_rcg_func_call_t rtc = codegen.gen_func(quantize_op, map_str_rtc_arg_t{{"out",       top_in},
                                                                                {"max_val",   make_scalar_nda(max_val)},
                                                                                {"drop_mask", make_scalar_nda(
                                                                                        drop_mask)}});
    ...
    void write_sigs(set_op_base_t &all_op_sigs, filename_t const &op_sigs_fn) {
        ...
        for (set_op_base_t::const_iterator i = all_op_sigs.begin(); i != all_op_sigs.end(); ++i) {
            (*out) << str(*i) << "\n";
        }
    }
    ...
    void conv_pipe_fwd_t::gen_op(p_conv_op_t const &cop) {
        ...
        if (oi->is(Concat_coi)) {
            ...
            if (oi->get_u32("emit_out_in_yx") == 1) {
                ...
                oi->set_null_arg_dims("out_in_yx",
                                      oi->get_dims("out")); // proper dims, but no var will be passed at call time
            }
            gen_call(oi);
            ...
            if (oi->get_dims("filts") != rtc->get_var_dims(filts_id)) { // ipconv uses untransformed filts, otherwise:
                oi->reset_arg("filts",
                              gen_apply_func_to_var("filts_ref", oi->get_arg("filts"), "filts", oi->get_dims("filts"),
                                                    "xpose_filts", oi));
            }
    ...
    void conv_pipe_fwd_t::gen_call(p_conv_op_t const &oi) {
        ...
        if (oi->is(Convolution_coi) && ((oi->get_func_name() == tconv_str) || (oi->get_func_name() == k1conv_str))) {
            must_insert(rfc->arg_map, "flags", make_scalar_nda(flags));
    ...
    void conv_pipe_fwd_t::gen_ops_rec(string const &node_name) {
        ...
        for (vect_string::const_iterator i = node->bot_for.begin(); i != node->bot_for.end(); ++i) {
            ...
            for (vect_string::const_iterator j = cop->tops.begin();
                 j != cop->tops.end(); ++j) {  // generate output nodes
                if (!cop->is(Convolution_coi)) {
                    gen_node_var(*j, *j);
                } // only if not conv (which explicitly/manually creates node var)
            }
            gen_op(cop);
    ...
    void conv_pipe_fwd_t::init(p_conv_pipe_t const &cp_, nesi_init_arg_t *const nia) {
        ...
        op_infos.reset(
                new map_str_p_conv_op_t); // maybe we should have our own copy of cp, but instead we only copy convs
        ...
        for (map_str_p_conv_op_t::iterator i = cp->convs->begin(); i != cp->convs->end(); ++i) {
            ...
            if (oi->is(Convolution_coi)) {
                ...
                if (oi->get_func_name() == k1conv_str) {
                    if ((no->in_place_ops.size() == conv_has_relu) &&
                        (no->bot_for.size() == 1)) { // if output feeds single non-in-place operation
        ...
        if (!rtc) {
            ...
            if (0) {}
                ...
                rt_err("rtc-fwd: can't find enabled choice for default backend. specify, or update defaults list ...");
            }
        ...
        for (set_string::const_iterator i = force_zero_names.begin();
             i != force_zero_names.end(); ++i) { rtc->set_var_to_zero(*i); }
        rtc->finish_and_sync();
    }

    void conv_pipe_fwd_t::run_fwd(vect_string const &to_set_vns, p_map_str_p_nda_float_t const &fwd,
        ...
        float const compute_dur = fwd_calls.empty() ? 0.0f : rtc->get_dur(fwd_calls.front().call_id,
                                                                          fwd_calls.back().call_id);
