...
namespace boda {
    struct cnn_custom_codegen_t : public custom_codegen_t {
        ...
        void gen_op_reduce(rtc_call_gen_t *rcg) {
            ...
            for (vect_arg_decl_t::multi_iter i = rcg->rtc_func_template->arg_decls.multi_begin(
                    &rcg->op); !i.at_end(); ++i) {
        ...
        string
        add_bias_then_maybe_relu(rtc_call_gen_t *rcg, dims_t const &work, uint32_t const &tx, uint32_t const ty) {
        ...
        void gen_op_bconv(rtc_call_gen_t *rcg) {
            ...
            for (uint32_t tx = 0; tx != work.dsz("out_ix"); ++tx) {
                rcg->line("loads",
                          strprintf("filts_strip[%s] = filts_smem[%%(LOC_ID_1D_out_ix_tile)*%%(work_out_ix_dim)+%s];",
                                    str(tx).c_str(), str(tx).c_str()));
            }
            for (uint32_t ty = 0; ty != work.dsz(
                    "pels"); ++ty) { // note: could merge with above loop, but we want to use ty for consistency
  ...
  if( igl_x >= 0 && igl_y >= 0 && igl_y < %(in_grad_loss_y_dim) && igl_x < %(in_grad_loss_x_dim) &&
    ...
    in_grad_loss[ %(pel_ix_img)*%(in_grad_loss_img_stride) + %(out_ix_in_chan)*%(in_grad_loss_chan_stride) + 
        ...
        void gen_op_bconv_fb(rtc_call_gen_t *rcg) {
            ...
            for (uint32_t tx = 0; tx != work.dsz("out_ix"); ++tx) {
                rcg->line("loads", strprintf(
                        "filts_strip[%s] = filts_smem[%%(LOC_ID_1D_out_ix_tile)*%%(work_fb_out_ix_dim)+%s];",
                        str(tx).c_str(), str(tx).c_str()));
            }
            for (uint32_t ty = 0; ty != work.dsz(
                    "pels"); ++ty) { // note: could merge with above loop, but we want to use ty for consistency
  ...
  if( %(pel_ix_in_chan) < %(filts_grad_loss_in_chan_dim) && %(out_ix_out_chan) < %(filts_grad_loss_out_chan_dim) ) {
    filts_grad_loss[ %(out_ix_out_chan)*%(filts_grad_loss_out_chan_stride) + %(pel_ix_in_chan)*%(filts_grad_loss_in_chan_stride) + 
        ...
        void gen_filts_smem_loads(rtc_call_gen_t *rcg,
                                  uint32_t const filts_smem_sz) { // note: filts_smem_sz must == tvv %(filts_smem_sz)
            ...
            for (uint32_t i = 0; i != out_chan_smem_load_iter; ++i) {
                ...
                if ((i + 1) * rcg->rtc_call_geom.tpb > filts_smem_sz) {
                    rcg->line("filts_smem_loads", "if( " + ixe + " < %(filts_smem_sz) ) {");
                    eif = "}";
                }
                // note: load is (always) contiguous
                rcg->line("filts_smem_loads",
                          strprintf("filts_smem[%s] = filts[filts_off+(%%(tpb)*%s)];%s", ixe.c_str(), str(i).c_str(),
                                    eif.c_str()));
            }
            ...
            rcg->line("biases_smem_loads",
                      "int32_t ocix; int32_t const ocix_base = %(GRP_ID_1D_out_chan_blk)*%(filts_x_stride);");
            for (uint32_t i = 0; i != out_chan_bias_smem_load_iter; ++i) {
                string const ixe = "(LOC_ID_1D + %(tpb) * " + str(i) + ")";
                string eif;
                rcg->line("biases_smem_loads", strprintf(
                        "ocix = ocix_base + (%s %%%% %%(work_out_chan_tile_dim))*%%(work_out_chan_dim) + ( %s / %%(work_out_chan_tile_dim) );",
                        ixe.c_str(), ixe.c_str()));
                if ((i + 1) * rcg->rtc_call_geom.tpb > filts_x_stride) {
                    rcg->line("biases_smem_loads", "if( " + ixe + " < %(filts_x_stride) ) {");
                    eif = "}";
                }
                // note: load is (always) contiguous
                rcg->line("biases_smem_loads",
                          strprintf("if( ocix < %%(biases_out_chan_dim) ) {filts_smem[%s] = biases[ocix];}%s",
                                    ixe.c_str(), eif.c_str()));
            }
        ...
        void gen_op_conv(rtc_call_gen_t *rcg) {
            ...
            uint32_t const pel_smem_load_iter = u32_ceil_div((work.dsz("pels") * work.dsz("pels_tile")),
                                                             rcg->rtc_call_geom.tpb);
            ...
            for (uint32_t tx = 0; tx != work.dsz("out_chan"); ++tx) {
                rcg->line("loads", strprintf(
                        "filts_strip[%s] = filts_smem[%%(LOC_ID_1D_out_chan_tile)+%s*%%(work_out_chan_tile_dim)];",
                        str(tx).c_str(), str(tx).c_str()));
            }
            for (uint32_t ty = 0; ty != work.dsz(
                    "pels"); ++ty) { // note: could merge with above loop, but we want to use ty for consistency
            ...
            for (uint32_t ty = 0; ty != work.dsz("out_chan"); ++ty) {
                rcg->line("stores",
                          strprintf("  tcix[%s] = (%%(out_chan_ix)+%s)*%%(out_chan_stride); // cache out chan ixs",
                                    str(ty).c_str(), str(ty).c_str()));
            }
            for (uint32_t ty = 0; ty != work.dsz("pels"); ++ty) {
                ...
                for (uint32_t tx = 0; tx != work.dsz("out_chan"); ++tx) {
                    rcg->line("fmas", strprintf("out_tile[%s] += filts_strip[%s]*in_strip[%s];",
                                                str((ty * work.dsz("out_chan") + tx)).c_str(), str(tx).c_str(),
                                                str(ty).c_str()));
                    rcg->line("stores", strprintf(
                            "if( tcix[%s] < (%%(out_chan_dim)*%%(out_chan_stride)) ) { out[ tpix[%s] + tcix[%s] ] = %s; }",
        ...
        void gen_op_ipconv(rtc_call_gen_t *rcg) {
            ...
            for (uint32_t i = 0; i != out_chan_smem_load_iter; ++i) {
                string const ixe = "(LOC_ID_1D + %(tpb) * " + str(i) + ")";
                string const filt_ix =
                        "( LOC_ID_1D/%(work_fioc_tile_dim) + %(tpb)/%(work_fioc_tile_dim)* " + str(i) + ")";
                string eif;
                ...
                rcg->line("filts_smem_loads",
                          strprintf("filts_smem[%s] = filts[filts_off+(%s*%%(filts_out_chan_stride))];%s", ixe.c_str(),
                                    filt_ix.c_str(), eif.c_str()));
            }
            ...
            for (uint32_t i = 0; i != in_smem_load_iter; ++i) {
                ...
                if ((i + 1) * rcg->rtc_call_geom.tpb > in_smem_sz) {
                    rcg->line("in_smem_loads", "if( " + ixe + " < %(in_smem_sz) ) {");
                    eif = "}";
                }
                rcg->line("in_smem_loads",
                          strprintf("in_smem[%s] = in[in_off+(%s*%%(in_img_stride))];%s", ixe.c_str(), img_ix.c_str(),
                                    eif.c_str()));
            }
            ...
            for (uint32_t tx = 0; tx != work.dsz("out_chan"); ++tx) {
                ...
                rcg->line("stores",
                          strprintf("if( (%%(LOC_ID_1D_fioc_tile) == 0 ) && ((ocix + %s) < %%(out_chan_dim)) ) "
        ...
        string gva(uint32_t const &vw, uint32_t const &ix) {
            ...
            return strprintf("[%s].%s%s", str(ix / vw).c_str(), maybe_s.c_str(),
                             string((vw <= 4) ? pss_alt : pss, ix % vw, 1).c_str());
        }
        ...
        void gen_op_sgemm_simd(rtc_call_gen_t *rcg) {
            ...
            assert_st((a_K_stride % vw) == 0);
            a_K_stride /= vw;
            assert_st((b_K_stride % vw) == 0);
            b_K_stride /= vw;
            for (uint32_t Kb = 0; Kb != work.dsz("Kb"); ++Kb) {
                for (uint32_t Mt = 0; Mt != work.dsz("Mt") / vw; ++Mt) {
                    rcg->line("inner_loop_body", strprintf("a_r[%s] = ((GASQ float%s const *)a)[a_off+%s];",
                                                           str(Mt).c_str(), str(vw).c_str(),
                                                           str(Mt + Kb * a_K_stride).c_str()));
                }
                for (uint32_t Nt = 0; Nt != work.dsz("Nt") / vw; ++Nt) {
                    rcg->line("inner_loop_body", strprintf("b_r[%s] = ((GASQ float%s const *)b)[b_off+%s];",
                                                           str(Nt).c_str(), str(vw).c_str(),
                                                           str(Nt + Kb * b_K_stride).c_str()));
                }
                for (uint32_t Mt = 0; Mt != work.dsz("Mt"); ++Mt) {
                    for (uint32_t Nt = 0; Nt != work.dsz("Nt"); ++Nt) {
                        uint32_t const rix = (Mt * work.dsz("Nt") + Nt);
        ...
        void gen_op_sgemm_no_local(rtc_call_gen_t *rcg) {
            ...
            for (uint32_t Kb = 0; Kb != work.dsz("Kb"); ++Kb) {
                for (uint32_t Mt = 0; Mt != work.dsz("Mt"); ++Mt) {
                    rcg->line("inner_loop_body",
                              strprintf("a_r[%s] = a[a_off+%s];", str(Mt).c_str(), str(Mt + Kb * a_K_stride).c_str()));
                }
                for (uint32_t Nt = 0; Nt != work.dsz("Nt"); ++Nt) {
                    rcg->line("inner_loop_body",
                              strprintf("b_r[%s] = b[b_off+%s];", str(Nt).c_str(), str(Nt + Kb * b_K_stride).c_str()));
                }
                for (uint32_t Mt = 0; Mt != work.dsz("Mt"); ++Mt) {
                    for (uint32_t Nt = 0; Nt != work.dsz("Nt"); ++Nt) {
                        uint32_t const rix = (Mt * work.dsz("Nt") + Nt);
        ...
        void gen_sgemm_sm_load(rtc_call_gen_t *rcg, string const &code_sec, string const &vn, uint64_t const &sm_sz,
            ...
            assert_st((stride % vw) == 0);
            stride /= vw;
            uint64_t const rows = sm_sz / row_len;
            assert_st(row_len * rows ==
                      sm_sz); // must be loading exactly an integer number of rows (not sensible otherwise?)
            ...
            for (uint32_t i = 0; i != sm_load_iter; ++i) {
                ...
                if (row_pad && (iter_row_off + rcg->rtc_call_geom.tpb >
                                row_len)) { // more than one row-per-block, need to add dynamic offset term
                    extra_off_str = strprintf("+(LOC_ID_1D+%s)/%s*%s", str(iter_row_off).c_str(),
                                              str(row_len).c_str(), str(row_pad).c_str());
                }

                string eif;
                if ((iter_sm_off + rcg->rtc_call_geom.tpb) >
                    sm_sz) {  // block extends past end of sm, need to add guard
                    rcg->line(code_sec,
                              strprintf("if( (LOC_ID_1D+%s) < %s ) {", str(iter_sm_off).c_str(), str(sm_sz).c_str()));
                    eif = "}";
                }
                string vn_vw = vn;
                assert_st(vw);
        ...
        void gen_op_sgemm(rtc_call_gen_t *rcg) {
            ...
            for (uint32_t Kb = 0; Kb != work.dsz("Kb"); ++Kb) {
                for (uint32_t Mt = 0; Mt != work.dsz("Mt"); ++Mt) {
                    rcg->line("inner_loop_body",
                              strprintf("a_r[%s] = a_sm_off[%s];", str(Mt).c_str(), str(Mt + Kb * blk_M).c_str()));
                }
                for (uint32_t Nt = 0; Nt != work.dsz("Nt"); ++Nt) {
                    rcg->line("inner_loop_body",
                              strprintf("b_r[%s] = b_sm_off[%s];", str(Nt).c_str(), str(Nt + Kb * blk_N).c_str()));
        ...
        void gen_sgemm_write_out(rtc_call_gen_t *rcg, bool const &half_to_float) {
            ...
            for (uint32_t Nt = 0; Nt != work.dsz("Nt"); ++Nt) {
                if (half_to_float) {
                    rcg->line("stores",
                              strprintf("vstore_half( b_r[%s], c_off+%s, c );", str(Nt).c_str(), str(Nt).c_str()));
                } else {
        ...
        void gen_op_k1conv_simd(rtc_call_gen_t *rcg) {
            ...
            assert_st((in_chan_stride % vw) == 0);
            in_chan_stride /= vw;
            assert_st((filts_in_chan_stride % vw) == 0);
            filts_in_chan_stride /= vw;
            ...
            for (uint32_t Kb = 0; Kb != Kb_dim; ++Kb) {
                for (uint32_t tx = 0; tx != work.dsz("pels") / vw; ++tx) {
                    rcg->line("inner_loop_body", strprintf("in_strip[%s] = ((GASQ float%s const *)in)[in_off+%s];",
                                                           str(tx).c_str(), str(vw).c_str(),
                                                           str(tx + Kb * in_chan_stride).c_str()));
                }
                for (uint32_t ty = 0; ty != work.dsz("out_chan") / vw; ++ty) {
                    rcg->line("inner_loop_body",
                              strprintf("filts_strip[%s] = ((GASQ float%s const *)filts)[filts_off+%s];",
                ...
                for (uint32_t tx = 0; tx != work.dsz("pels"); ++tx) {
                    for (uint32_t ty = 0; ty != work.dsz("out_chan"); ++ty) {
                        ...
                        rcg->line("inner_loop_body",
                                  strprintf("out_tile[%s] += in_strip%s*filts_strip%s;", str(rix).c_str(),
        ...
        void gen_op_conv_simd(rtc_call_gen_t *rcg) {
            ...
            assert_st(stride.dsz("y") ==
                      stride.dsz("x")); // FIXME/NOTE: see uniform stride FIXMEs in kernel and cnn_op.cc
            ...
            for (uint32_t Kb = 0; Kb != Kb_dim; ++Kb) {
                for (uint32_t tx = 0; tx != work.dsz("pels"); ++tx) {
                    ...
                    string reo;
                    if (row_extra_off) {
                        reo = strprintf("+((out_x + %s)/%%(out_pels_x_dim)*%s)", str(tx).c_str(),
                                        str(row_extra_off).c_str());
                    }
                    rcg->line("inner_loop_body", strprintf("in_strip%s = in[in_off+%s %s];",  // FIXME: use Kb
                                                           gva(vw, tx).c_str(), str(tx * stride.dsz("x")).c_str(),
                                                           reo.c_str()));
                }
                for (uint32_t ty = 0; ty != work.dsz("out_chan") / vw; ++ty) {
                    rcg->line("inner_loop_body",
        ...
        void gen_op_k1conv(rtc_call_gen_t *rcg) {
            ...
            dims_t const &work = rcg->get_arg_dims_by_name("work");
            dims_t const &filts = rcg->get_arg_dims_by_name("filts");
            assert_st(filts.dsz("x") == 1);
            assert_st(filts.dsz("y") == 1);
            ...
            rcg->set("filts_smem_sz", str(filts_smem_sz));
            uint32_t const out_smem_sz = work.dsz("pels_tile") * work.dsz("out_chan_tile") *
                                         work.dsz("pels"); // note: == oi->tpb*work.dsz("pels")
            rcg->set("out_smem_sz", str(out_smem_sz)); // note: unused, but assumed that all_smem_sz >= out_smem_sz
            uint32_t const all_smem_sz = std::max(out_smem_sz, filts_smem_sz + in.dstride(
                    "blk_iter")); // note: %(in_blk_iter_sz) == in_smem_sz
            ...
            for (uint32_t i = 0; i != in_smem_load_iter; ++i) {
                string const ixe = "(LOC_ID_1D + %(tpb) * " + str(i) + ")";
                string eif;
                if ((i + 1) * rcg->rtc_call_geom.tpb > in.dstride("blk_iter")) {
                    rcg->line("smem_loads", "if( " + ixe + " < %(in_blk_iter_stride)) { ");
                    eif = "}";
                }
            ...
            rcg->set("out_chan_tile",
                     "(%(GRP_ID_1D_out_chan_blk)*%(work_out_chan_tile_dim)+%(LOC_ID_1D_out_chan_tile))");
            rcg->set("out_chan_ix", "(%(out_chan_tile)*%(work_out_chan_dim))");
            ...
            if (out.get_dim_by_name(
                    "blk")) { // aka if(write_xposed) -- if this dim names exists in the output, we know to write in xposed format
                ...
                assert_st(work.dsz("out_chan_blk") * work.dsz("out_chan_tile") * work.dsz("out_chan") ==
                          out.dsz("blk_iter") * out.dsz("blk_iter_chan"));
                ...
                assert_st(work.dsz("pels_blk") * work.dsz("pels_tile") * work.dsz("pels") ==
                          out.dsz("blk") * out.dsz("blk_pel"));
                ...
                rcg->line("stores",
                          "int32_t const out_ix = (%(GRP_ID_1D_out_chan_blk)*%(work_out_chan_tile_dim)*%(work_out_chan_dim))*%(out_blk_iter_chan_stride) + %(GRP_ID_1D_pels_blk)*%(out_blk_stride);");
                ...
                for (uint32_t tx = 0; tx != work.dsz("out_chan"); ++tx) {
                    ...
                    rcg->line("stores", "  BARRIER_SYNC;");
                    for (uint32_t ty = 0;
                         ty != work.dsz("pels"); ++ty) { // all_smem[] -> [xpbuf[] (registers)] -> out[] (global)
                        ...
                        rcg->line("stores", strprintf(
                                "out[out_ix + xpbuf_rd_pel + (xpbuf_rd_chan*%%(work_out_chan_dim)+%s)*%%(out_blk_iter_chan_stride)] = "
                ...
                for (uint32_t ty = 0; ty != work.dsz("pels"); ++ty) {
                    insert_nda_ix_exprs(rcg->tsvs, "out_pel_" + str(ty), must_find(rcg->all_ix_dims, "out_ref_pel"),
                                        "( (%(GRP_ID_1D_pels_blk)*%(work_pels_tile_dim) + %(LOC_ID_1D_pels_tile))*%(work_pels_dim) + " +
                                        str(ty) + " )");
            ...
            for (uint32_t ty = 0; ty != work.dsz("pels"); ++ty) {
                for (uint32_t tx = 0; tx != work.dsz("out_chan"); ++tx) {
                    rcg->line("dummy_stores", strprintf("out_off[%s] = %s;",
                                                        str((ty * work.dsz("out_chan") + tx) *
                                                            rcg->rtc_call_geom.tpb).c_str(),
                                                        add_bias_then_maybe_relu(rcg, work, tx, ty).c_str()));
                }
            }
            for (uint32_t tx = 0; tx != work.dsz("out_chan"); ++tx) {
            ...
            for (uint32_t ict = 0; ict != in.dsz("blk_iter_chan"); ++ict) {
                for (uint32_t tx = 0; tx != work.dsz("out_chan"); ++tx) {
                    rcg->line("inner_loop_body", strprintf(
                            "filts_strip[%s] = filts_smem_off[(%s*%%(filts_in_chan_stride))+%s*%%(work_out_chan_tile_dim)];",
        ...
        void gen_op_tconv(rtc_call_gen_t *rcg) {
            ...
            uint32_t const in_smem_load_iter = u32_ceil_div(in.dstride("blk_in_chan"),
                                                            rcg->rtc_call_geom.tpb);  // in smem loads
            for (uint32_t i = 0; i != in_smem_load_iter; ++i) {
                string const ixe = "(LOC_ID_1D + %(tpb) * " + str(i) + ")";
                string eif;
                if ((i + 1) * rcg->rtc_call_geom.tpb > in.dstride("blk_in_chan")) {
                    rcg->line("in_smem_loads", "if( " + ixe + " < %(in_blk_in_chan_stride)) { ");
                    eif = "}";
                }
                rcg->line("in_smem_loads",
                          strprintf("in_smem[%s] = in[ blk_in_ix_base + (%%(tpb)*%s) ];%s", ixe.c_str(), str(i).c_str(),
                                    eif.c_str()));
            }
            ...
            for (uint32_t i = 0; i != in.dsz("blk_x"); ++i) {
                rcg->line("inner_loop_body",
                          strprintf("in_strip[%s] = in_smem_off[%s];", str(i).c_str(), str(i).c_str()));
            }
            ...
            for (uint32_t kx = 0; kx != filts.dsz("x"); ++kx) {
                for (uint32_t tx = 0; tx != work.dsz("out_chan"); ++tx) {
                    rcg->line("inner_loop_body", strprintf(
                            "filts_strip[%s] = filts_smem_off[%s*%%(filts_x_stride)+%s*%%(filts_out_chan_reg_stride)];",
                ...
                for (uint32_t ty = 0; ty != work.dsz("pels"); ++ty) {
                    for (uint32_t tx = 0; tx != work.dsz("out_chan"); ++tx) {
                        rcg->line("inner_loop_body", strprintf("out_tile[%s] += filts_strip[%s]*in_strip[%s];",
                                                               str((ty * work.dsz("out_chan") + tx)).c_str(),
                                                               str(tx).c_str(),
                                                               str(ty * stride.dsz("x") + kx).c_str()));
                    }
                }
            }
            for (uint32_t tx = 0; tx != work.dsz("out_chan"); ++tx) {
                rcg->line("bias_loads", strprintf("filts_strip[%s] = filts_smem_off[%s*%%(filts_out_chan_reg_stride)];",
                                                  str(tx).c_str(), str(tx).c_str()));
            }
            ...
            rcg->line("stores",
                      "GASQ float * out_off = out + %(out_line_img)*%(out_img_stride) + out_chan*%(out_chan_stride) + "
