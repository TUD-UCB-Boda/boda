...
namespace boda_caffe {
    bool layer_included_for_state(caffe::LayerParameter const &layer_param, caffe::NetState const &net_state);
}
...
namespace boda {
    ...
    void set_param_from_conv_op(CP &cp, p_conv_op_t conv_op) {
        ...
        if (conv_op->has("in_pad")) {
            ...
            if (pad.dims_are_same()) { cp.add_pad(pad.d[0]); }
                ...
                cp.set_pad_w(pad.d[0]);
                cp.set_pad_h(pad.d[1]);
            }
        }
        ...
        cp.clear_kernel_w();
        cp.clear_kernel_h();
        cp.clear_kernel_size();
        if (kern_sz.dims_are_same()) { cp.add_kernel_size(kern_sz.d[0]); }
        else {
            cp.set_kernel_w(kern_sz.d[0]);
            cp.set_kernel_h(kern_sz.d[1]);
        }
        ...
        if (conv_op->has("stride")) {
            ...
            if (conv_op->stride().dims_are_same()) { cp.add_stride(conv_op->stride().d[0]); }
                ...
                cp.set_stride_w(conv_op->stride().d[0]);
                cp.set_stride_h(conv_op->stride().d[1]);
            }
        }
    }

    template void
    set_param_from_conv_op<caffe::ConvolutionParameter>(caffe::ConvolutionParameter &cp, p_conv_op_t conv_op);

    void multi_dim_err(uint32_t const &num, string const &tag) {
        rt_err(strprintf(
                "saw %s %s values. arbitrary # of spatial dims case not supported. use 1 or 2 %s values or %s_w/%s_h.",
    ...
    void fill_in_conv_op_from_param(p_conv_op_t const &conv_op, caffe::ConvolutionParameter const &cp) {
        ...
        if (!(cp.has_pad_w() || cp.has_pad_h())) {
            ...
            else if (cp.pad_size() == 1) {
                conv_op->set_dims("in_pad", dims_t{{cp.pad(0), cp.pad(0)}, {"y", "x"}, "none"});
            }
            else if (cp.pad_size() == 2) {
                conv_op->set_dims("in_pad", dims_t{{cp.pad(0), cp.pad(1)}, {"y", "x"}, "none"});
            }
            else {
                multi_dim_err(cp.pad_size(), "in_pad");
            } // FIXME: we could handle this now as a first step for N-D support
        } else {
            assert_st(cp.has_pad_w() && cp.has_pad_h() && (!cp.pad_size()));
        ...
        if (!(cp.has_stride_w() || cp.has_stride_h())) {
            ...
            else if (cp.stride_size() == 1) {
                conv_op->set_dims("stride", dims_t{{cp.stride(0), cp.stride(0)}, {"y", "x"}, "none"});
            }
            else if (cp.stride_size() == 2) {
                conv_op->set_dims("stride", dims_t{{cp.stride(0), cp.stride(1)}, {"y", "x"}, "none"});
            }
            else {
                multi_dim_err(cp.stride_size(), "stride");
            } // FIXME: we could handle this now as a first step for N-D support
        } else {
            assert_st(cp.has_stride_w() && cp.has_stride_h() && (!cp.stride_size()));
        ...
        if (!(cp.has_kernel_w() || cp.has_kernel_h())) {
            ...
            else if (cp.kernel_size_size() == 1) {
                conv_op->set_dims("kern_sz", dims_t{{cp.kernel_size(0), cp.kernel_size(0)}, {"y", "x"}, "none"});
            }
            else if (cp.kernel_size_size() == 2) {
                conv_op->set_dims("kern_sz", dims_t{{cp.kernel_size(0), cp.kernel_size(1)}, {"y", "x"}, "none"});
            }
            else { multi_dim_err(cp.kernel_size_size(), "kernel_size"); }
        } else {
            assert_st(cp.has_kernel_w() && cp.has_kernel_h() && (!cp.kernel_size_size()));
    ...
    void fill_in_conv_op_from_param(p_conv_op_t const &conv_op, caffe::PoolingParameter const &cp) {
        ...
        if (!(cp.has_pad_w() || cp.has_pad_h())) {
            ...
            assert_st(cp.has_pad_w() && cp.has_pad_h() && (!cp.has_pad()));
            conv_op->set_dims("in_pad", dims_t{{cp.pad_h(), cp.pad_w()}, {"y", "x"}, "none"});
        }
        if (!(cp.has_stride_w() || cp.has_stride_h())) {
            conv_op->set_dims("stride", dims_t{{cp.stride(), cp.stride()}, {"y", "x"}, "none"});
        } else {
            assert_st(cp.has_stride_w() && cp.has_stride_h() && (!cp.has_stride()));
            conv_op->set_dims("stride", dims_t{{cp.stride_h(), cp.stride_w()}, {"y", "x"}, "none"});
        }
...
#define RF_TO_VEC(V, RF) { for( int32_t i = 0; i != RF##_size(); ++i ) { V.push_back( RF(i) ); } }
    ...
    void net_input_err(p_net_param_t const &net_param) {
        rt_err(strprintf(
                "error: top-level input blob specification error. the number of input blobs (net_param->input_size()=%s) must be equal either to the number of input shapes (net_param->input_shape_size()=%s) or to the number of input dims (net_param->input_dim_size()=%s) divided by four. also, only one of the two shape-specification methods (input_dim or input_shape) may be used at the same time. \n",
                str(net_param->input_size()).c_str(), str(net_param->input_shape_size()).c_str(),
                str(net_param->input_dim_size()).c_str()));
    }
    ...
    void add_data_img_node(p_conv_pipe_t const &conv_pipe, string const &data_img_node_name,
                           dims_t const &data_img_node_dims) {
        ...
        if (conv_pipe->data_num_imgs.v != data_img_node_dims.dsz("img")) {
            rt_err("unhandled: multiple data layers with differing numbers of images.");
        }
    ...
    p_conv_pipe_t create_pipe_from_param(p_net_param_t const &net_param, map_str_uint32_t const &in_dims,
        ...
        for (int32_t in_ix = 0; in_ix != net_param->input_size(); ++in_ix) {
            ...
            if (input_dims.size() == 4) {
                input_dims.names(0) = "img";
                input_dims.names(1) = "chan";
                input_dims.names(2) = "y";
                input_dims.names(3) = "x";
            ...
            if (add_bck_ops) {
                printf("warning: add_bck_ops + top-level input blobs unhandled ... label input may be missing");
            }
        }
        ...
        for (int32_t i = 0; i != net_param->layer_size(); ++i) {
            ...
            if (0) {
                ...
                assert_st(conv_op->has(
                        "kern_sz")); // FIXME: convolutions *must* specify kernel size, i think? check in caffe
                ...
                if (!add_bck_ops) { // if not adding bck ops, treat dropout as no-op
                    if (conv_op->tops !=
                        conv_op->bots) { // not in place, so replace with clone operation (FIXME: use reshape?)
                        conv_op->erase_type();
                        conv_op->set_type(clone_coi.type);
                ...
                if (!add_bck_ops) {
                    ...
                    if (conv_op->tops.size() > 1) {
                        rt_err("expected 0 or 1 outputs for SoftmaxWithLoss (i.e. just a loss output or nothing (an implicit missing loss output)). saw '" +
                               str(conv_op->tops.size()) + "' outputs.");
                    }
                    if (conv_op->tops.size() == 0) {
                        rt_err("FIXME: restore loss auto-naming or fix prototxts");
                        conv_op->tops.push_back("loss");
                    }
                    conv_op->tops.insert(conv_op->tops.begin(),
                                         conv_op->bots[0] + "_grad_loss"); // add gradient output for fwd_top input
                }
                ...
                assert_st(
                        conv_op->has("kern_sz") != pp.global_pooling()); // global pooling iff no kernel size specified
            ...
            for (int32_t i = 0; i != lp.top_size(); ++i) {
                if (out_node_name == lp.top(i)) {
                    layer_has_out_node = 1;
                    found_out_node = 1;
                }
            }
...
#undef RF_TO_VEC
    ...
    parse_datum_into(p_nda_float_t &out, uint32_t const out_ix, void const *const bytes, uint32_t const bytes_sz) {
        ...
        if (chans * hi * wi != caffe_datum.data().size()) {
            rt_err(strprintf(
                    "inconsistency in datum data size vs datum dims: chans=%s hi=%s wi=%s so chans*h*w=%s but caffe_datum.data().size()=%s\n",
                    str(chans).c_str(), str(hi).c_str(), str(wi).c_str(), str(chans * hi * wi).c_str(),
                    str(caffe_datum.data().size()).c_str()));
        }
    ...
    p_net_param_t must_read_binary_proto(filename_t const &fn, filename_t const &alt_fn) {
        ...
        if (!ret) {
            rt_err(strprintf("failed to parse Netparamter from binary prototxt file %s",
    ...
    void create_identity_weights(p_net_param_t net, p_conv_pipe_t pipe, string const &layer_name,
                                 uint32_t const noise_mode) {
        ...
        for (dims_iter_t di(biases->dims);;) {
            biases->at(di.di) = 0;
            if (!di.next()) { break; }
        } // all biases 0
    ...
    void resize_kernel(p_nda_float_t const &in, p_nda_float_t const &out) {
        ...
        for (uint32_t i = 0; i != in->dims.dims(0); ++i) {
            for (uint32_t j = 0; j != in->dims.dims(1); ++j) {
                //print_kernel( in, i, j );
                for (uint32_t y = 0; y != in_ksz.d[1]; ++y) {
                    resize_1d(&in->at3(i, j, y), in_ksz.d[0], &kbuf[y * out_ksz.d[0]], out_ksz.d[0]);
                }
    ...
    struct cnet_mod_t : virtual public nesi // NESI(help="base class for utilities to modify caffe nets" )
    {
        ...
        void ensure_out_dir(nesi_init_arg_t *const nia) {
            ensure_is_dir(nesi_filename_t_expand(nia, "%(models_dir)/%(out_model)"), 1);
        }
        ...
        void write_mod_net(void) {
            ...
            if (!ret) {
                rt_err(strprintf("failed to write NetParamter to binary prototxt file %s",
    ...
    void create_upsamp_layer_weights(p_conv_pipe_t const &conv_pipe, string const &cpln,
        ...
        assert_st(usl_blobs->at(1)->dims ==
                  usl_blobs_upsamp->at(1)->dims); // biases should be same shape (and same strides?)
        ...
        for (dims_iter_t di(usl_blobs_upsamp->at(0)->dims);;) {
            usl_blobs_upsamp->at(0)->at(di.di) = 0;
            if (!di.next()) { break; }
        }

        for (dims_iter_t di(usl_blobs->at(0)->dims);;) {
            usl_blobs_upsamp->at(0)->at4(di.di[0], di.di[1], di.di[2] >> 1, di.di[3] >> 1) += usl_blobs->at(0)->at(
                    di.di);
            if (!di.next()) { break; }
        }
        ...
        // bases=["cnet_mod_t","has_main_t"], type_id="cnet_fc_to_conv")
    {
        ...
        void main(nesi_init_arg_t *nia) {
            ...
            trained_net = must_read_binary_proto(trained_fn,
                                                 filename_t()); // we need to load the original weights 'early' to infer input dims
        ...
        void fc_weights_to_conv_weights(string const &layer_name) {
            ...
            assert_st(blobs.size() == 2); // filters, biases
            assert_st(blobs_mod.size() == 2); // filters, biases
            printf("blobs[1]->dims=%s blobs_mod[1]->dims=%s\n", str(blobs[1]->dims).c_str(),
                   str(blobs_mod[1]->dims).c_str());
            // assert_st( blobs[1]->dims == blobs_mod[1]->dims ); // biases should be same shape (and same strides?) too strong
            assert_st(blobs[1]->dims.dims_prod() == blobs_mod[1]->dims.dims_prod());
            blobs_mod[1]->copy_elems_with_reshape(*blobs[1]); // reshape
        ...
        // bases=["cnet_mod_t","has_main_t"], type_id="cnet_util")
    {
        ...
        void main(nesi_init_arg_t *nia) {
            ...
            *new_conv_layer = net_param->layer(add_before_ix); // start with clone of layer we're adding before
            ...
            new_conv_layer->clear_bottom();
            new_conv_layer->add_bottom(pre_layer_top);
            new_conv_layer->clear_top();
            new_conv_layer->add_top(new_layer_name);
            ...
            *new_relu_layer = post_relu_layer; // start with clone of RELU from after layer we're adding before
            ...
            new_relu_layer->clear_bottom();
            new_relu_layer->add_bottom(new_layer_name);
            new_relu_layer->clear_top();
            new_relu_layer->add_top(new_layer_name);
