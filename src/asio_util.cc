...
namespace boda {
    ...
    string get_boda_shm_filename(void) {
        return strprintf("/boda-rev-%s-pid-%s-top.shm", get_build_rev(),
                         str(getpid()).c_str());
    }
    ...
    int get_single_parent_fd(string const &bpa) {
        ...
        if (fd0 != fd1) {
            rt_err("this worker only supports the case where the read and write fds are the same (the socketpair() case)");
        }
        return fd0;
    }
    ...
    struct udp_stream_t {
        ...
        void recv_msg(void) {
            ...
            ssize_t recv_sz = recvfrom(listen_fd, &msg_buf[0], msg_buf.size(), 0, (sockaddr *) &src_addr,
                                       &src_addr_len);
            if (recv_sz == -1) { rt_err(strprintf("recvfrom failed: %s", strerror(errno))); }
            if (recv_sz == (ssize_t) msg_buf.size()) {
                rt_err("recieved unexpected too-large datagram of size " + str(recv_sz));
            }
