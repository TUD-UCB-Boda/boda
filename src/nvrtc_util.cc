...
namespace boda {
    ...
    void nvrtc_err_chk(nvrtcResult const &ret, char const *const func_name) {
        if (ret != NVRTC_SUCCESS) {
            rt_err(strprintf("%s() failed with ret=%s (%s)", func_name, str(ret).c_str(), nvrtcGetErrorString(ret)));
        }
    }

    void nvrtcDestroyProgram_wrap(nvrtcProgram p) {
        if (!p) { return; }
        nvrtc_err_chk(nvrtcDestroyProgram(&p), "nvrtcDestroyProgram");
    }
...
#undef CU_GET_FUNC_ATTR_HELPER_MACRO
    ...
    struct var_info_t {
        ...
        var_info_t(dims_t const &dims_) : cup(make_shared<cup_t>(dims_.bytes_sz())),
                                          dims(dims_) {} // , ev( make_p_CUevent() ) { }
...
#define store_float_to_rp_float( val, ix, p ) p[ix] = val
        ...
        // bases=["rtc_compute_t"], type_id="nvrtc" )
    {
        ...
        virtual string get_plat_tag(void) {
            ...
            string dn;
            dn.resize(256);
            cu_err_chk(cuDeviceGetName(&dn[0], dn.size() - 1, cu_dev),
                       "cuDeviceGetName"); // FIXME: docs unclear if max size includes NULL
        ...
        void compile(vect_rtc_func_info_t const &func_infos, rtc_compile_opts_t const &opts) {
            ...
            if (gen_src) {
                write_whole_fn(
                        strprintf("%s/out_%s.cu", gen_src_output_dir.exp.c_str(), str(compile_call_ix.v).c_str()),
                        cucl_src);
            }
            string const prog_ptx = nvrtc_compile(cucl_src, opts);
            if (gen_src) {
                write_whole_fn(
                        strprintf("%s/out_%s.ptx", gen_src_output_dir.exp.c_str(), str(compile_call_ix.v).c_str()),
                        prog_ptx);
            }
            CUmodule new_cu_mod;
        ...
        string nvrtc_compile(string const &cuda_prog_str, rtc_compile_opts_t const &opts) {
            ...
            auto const comp_ret = nvrtcCompileProgram(cuda_prog.get(), cc_opts.size(),
                                                      &get_vect_rp_const_char(cc_opts)[0]);
            string const log = nvrtc_get_compile_log(cuda_prog);
            if (opts.show_compile_log) { printf("NVRTC COMPILE LOG:\n%s\n", str(log).c_str()); }
            nvrtc_err_chk(comp_ret,
        ...
        virtual float get_dur(uint32_t const &b, uint32_t const &e) {
            ...
            cu_err_chk(cuEventElapsedTime(&compute_dur, *get_call_ev(b).b_ev, *get_call_ev(e).e_ev),
                       "cuEventElapsedTime");
            return compute_dur;
        }
        ...
        uint32_t run(rtc_func_call_t const &rfc) {
            ...
            for (vect_string::const_iterator i = nfi.info.arg_names.begin(); i != nfi.info.arg_names.end(); ++i) {
                ...
                if (ai == rfc.arg_map.end()) {
                    rt_err(strprintf("nvrtc_compute_t: arg '%s' not found in arg_map for call.\n",
                                     str((*i)).c_str()));
                }
    ...
    typedef shared_ptr<nvrtc_compute_t> p_nvrtc_compute_t;
