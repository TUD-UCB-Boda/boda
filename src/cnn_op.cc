...
namespace boda {
    ...
    void add_cnn_codegen_annotations(conv_op_base_t *const op, op_tune_t const &op_tune_,
        ...
        if (!op->is(Concat_coi)) {
            ni_dims = op->get_dims(op->coi->bot_an(0));
        } // empty for Concat where we shouldn't use it, otherwise first input
        bool const is_conv = op->is(Convolution_coi);
        bool const is_pool = op->is(Pooling_coi);
        dims_t in_dims;
        if (op->is(Dropout_coi) || op->is(BckDropout_coi)) {
            op->set_dims("det_drop_seed", make_scalar_dims_t("uint32_t"));
        }
        ...
        if (is_conv || is_pool || op->is(Spreading_coi) || op->is(BckConv_coi)) {
            ...
            if (op->has("kern_sz")) { kern_sz_ = op->kern_sz(); }
                ...
                op->set_dims("kern_sz",
                             dims_t{{kern_sz_.d[1], kern_sz_.d[0]}, {"y", "x"}, "none"}); // FIXME: not ideal ...
            }
                ...
                if (op_tune->use_culibs) {
                    ...
                    if (!op->in_pad().is_zeros()) {
                        ...
                        if (op_tune->use_local_mem == 2) { op->set_func_name(k1conv_simd_str); }
                                                                   ...
                                                                   && (kern_sz_.both_dims_ge(u32_pt_t{1, 1}) &&
                                                                       (no_sz.d[0] >= 6))))) {
            ...
            if (op->is(BckConv_coi)) {
                ...
                bck_in_off -= bck_in_pad *
                              u32_to_i32(op->stride()); // first region calculated at - corner of padding out space
                // calculate number of extra pels needed to cover last pel in unpadded input space
                i32_pt_t bck_pels_sz = ceil_div(
                        u32_to_i32(get_xy_dims(no_dims)) - (bck_in_off + u32_to_i32(op->stride())), op->stride());
                ...
                op->set_dims("bck_pad_in_off",
                             dims_t(vect_uint32_t{bck_pad_in_off.d[1], bck_pad_in_off.d[0]}, vect_string{"y", "x"},
                                    "none"));
                ...
                op->set_dims("pix", dims_t(vect_uint32_t{no_dims.dsz("img"),
                                                         uint32_t(bck_pels_sz.d[1]), uint32_t(bck_pels_sz.d[0])},
                                           vect_string{"img", "y", "x"}, "none"));
                gbt.init(t_tile_sz, max_tpb,
                         u32_pt_t(op->get_dims("pix").dims_prod(), op->get_dims("oix").dims_prod()));
                dims_t work;
                work.tn = "none";
                ...
                op->set_dims("fioc",
                             dims_t(vect_uint32_t{ogld.dsz("chan"), u32_ceil_div(kern_sz_.d[1], op->stride().d[1]),
                                                  u32_ceil_div(kern_sz_.d[0], op->stride().d[0])},
                                    vect_string{"out_chan", "ky", "kx"}, "none"));

                gbt_tile_t gbt_fb;
                gbt_fb.init(t_tile_sz, max_tpb,
                            u32_pt_t(fgld.dsz("in_chan") * fgld.dsz("y") * fgld.dsz("x"), fgld.dsz("out_chan")));
                dims_t work_fb;
                work_fb.tn = "none";
            ...
            if (is_conv && (op->get_func_name() != "cudnn_conv")) { // skip blocking setup for cudnn_conv
                ...
                if ((op->get_func_name() == tconv_str) || (op->get_func_name() == k1conv_str)) {
                    op->set_dims("flags", make_scalar_dims_t(
                            "uint32_t")); // exactly these two variants have this debugging input
                }
                if (op->get_func_name() == tconv_str) {
                    ...
                    in_dims = dims_t(vect_uint32_t{
                                             work.dsz("blk_bline"), work.dsz("blk_bx"), ni_dims.dsz("chan"), tconv_blk_max_in_lines,
                                             tconv_blk_x_sz},
                ...
                if (op->get_func_name() == k1conv_str) {
                    ...
                    in_dims = dims_t(vect_uint32_t{
                                             work.dsz("pels_blk"), u32_ceil_div(ni_dims.dsz("chan"), in_blk_iter_chan_dim),
                                             in_blk_iter_chan_dim, work.dsz("pels_tile") * work.dsz("pels")},
                    ...
                    if ((work.dsz("pels") % op_tune->vw) != 0) {
                        unsup_err("k1conv_simd only supports work.pels being a multiple of vw");
                    }
                    if ((work.dsz("out_chan") % op_tune->vw) != 0) {
                        unsup_err("k1conv_simd only supports work.out_chan being a multiple of vw");
                    }
                    ...
                    uint32_t const out_chan_pad =
                            work.dsz("out_chan_blk") * work.dsz("out_chan_tile") * work.dsz("out_chan");
                    assert_st(out_chan_pad >= op->get_dims("filts").dsz("out_chan"));
                    // FIXME: pad in_chan to multiple of Kb?
                    op->set_u32("Kb", op_tune->Kb);
                    uint32_t in_chan_pad = ni_dims.dsz(
                            "chan"); // not padded yet but may be layer; note: == filts in_chan dim
                    ...
                    op->set_u32("vw", op_tune->vw);
                    if ((work.dsz("pels") % op_tune->vw) != 0) {
                        unsup_err("conv_simd only supports work.pels being a multiple of vw");
                    }
                    if ((work.dsz("out_chan") % op_tune->vw) != 0) {
                        unsup_err("conv_simd only supports work.out_chan being a multiple of vw");
                    }
                    if (op_tune->Kb != 1) {
                        unsup_err("conv_simd only supports Kb == 1");
                    } // FIXME: for now, no inner loop unroll ...
                    // FIXME: pad in_chan to multiple of Kb?
                    ...
                    op->set_dims("out_pels", dims_t(vect_uint32_t{in_dims.dsz("img"), out_xy.d[1], out_xy.d[0]},
                                                    vect_string{"img", "y", "x"},
                                                    "none")); // min output pels we must calculate
                    ...
                    work.add_dims("pels", gbt.mn_per_thr.d[0], "out_chan",
                                  gbt.mn_per_thr.d[1]); // dims of per-thread work
                    work.calc_strides();

                    uint32_t const pels_sz_pad =
                            work.dsz("pels_blk") * work.dsz("pels_tile") * work.dsz("pels"); // final pels we will calc
                    ...
                    uint32_t final_in_pels_pad = u32_ceil_align(
                            op->get_dims("in_pels").dims_prod() + filt_dep_extra_in_pad, op_tune->vw);
                    in_dims = dims_t(vect_uint32_t{in_chan_pad, final_in_pels_pad}, vect_string{"chan", "pel"},
                                     in_dims.tn);
                    // note: for now, we don't pad and/or xpose out, so the store code must handle that.
                    op->reset_dims("filts", dims_t(
                            vect_uint32_t{in_chan_pad, kern_sz_.d[1], kern_sz_.d[0], out_chan_pad},
                ...
                if (is_k1_or_t_or_reg_conv(op->get_func_name())) {
                    op->reset_dims("filts", dims_t(vect_uint32_t{work.dsz("out_chan_blk"), ni_dims.dsz("chan"),
                                                                 ...
                                                                 work.dsz("out_chan"), work.dsz("out_chan_tile")},
                                                   vect_string{"out_chan_blk", "in_chan", "y", "x",
        ...
        else if (op->is(ZeroIfNonPos_coi)) {
            op->set_func_name(op->get_type());
        } // wow! one with a consistent name! sigh.
            ...
            if (op_tune.use_culibs) {
                ...
                if (op_tune.prof_variant) { anno_op->set_func_name("sgemm_prof"); }
                    ...
                    if (0) {}
                        ...
                        rt_err(strprintf("unknonw value for op_tune.use_local_mem of %s\n",
