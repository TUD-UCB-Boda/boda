...
#include"nesi_decls.H"
    ...
    int boda_main_arg_proc(ostream &os, int argc, char **argv) {
        ...
        if (do_help) {
            if (help_args.empty()) {
                ...
                nesi_struct_hier_help(&cinfo_has_main_t, &out, prefix, help_all);
                os << out;
            } else {
                std::string const help_for_mode = help_args.front();
                help_args.erase(help_args.begin());
                p_lexp_t lexp = help_ex ? parse_lexp(help_for_mode) : make_list_lexp_from_one_key_val("mode",
                                                                                                      help_for_mode);
                p_has_main_t has_main;
                lexp_name_val_map_t nvm(lexp);
                nesi_struct_make_p(&nvm, &tinfo_has_main_t, &has_main);
                nesi_struct_nesi_help(&tinfo_has_main_t, has_main.get(), &out, prefix, help_all, &help_args, 0);
                os << out;
            }
            return 1;
        }
    ...
    int boda_main_wrap(int argc, char **argv) {
        ...
        catch (rt_exception const &e) {
            printstr(e.what_and_stacktrace());
            ret = e.get_ret_code();
        }
